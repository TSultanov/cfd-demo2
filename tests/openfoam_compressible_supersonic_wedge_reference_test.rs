...
fn openfoam_compressible_supersonic_wedge_matches_reference_field() {
    ...
    let table = common::load_csv(&common::data_path(
        "compressible_supersonic_wedge_full_field.csv",
    ));
    ...
    let mut sol_map: HashMap<(i64, i64), (f64, f64, f64)> =
        HashMap::with_capacity(mesh.num_cells());
    for i in 0..mesh.num_cells() {
        sol_map.insert(
            yx_key(mesh.cell_cx[i], mesh.cell_cy[i]),
            (p[i], u[i].0, u[i].1),
        );
    }

    let mut ref_map: HashMap<(i64, i64), (f64, f64, f64)> =
        HashMap::with_capacity(mesh.num_cells());
    ...
    assert!(
        p_ref_dev > 1e3 && uy_ref_max > 5.0,
        "reference appears trivial: p_dev={p_ref_dev:.3e} max_abs(u_y)={uy_ref_max:.3e}"
    );
    assert!(
        p_sol_dev > 1e2 && uy_sol_max > 1.0,
        "solver appears trivial: p_dev={p_sol_dev:.3e} max_abs(u_y)={uy_sol_max:.3e}"
    );

    if common::diag_enabled() {
        let p_ref_mean = common::mean(&p_ref);
        let p_sol_mean = common::mean(&p_sol);
        let p_ref_min = p_ref.iter().copied().fold(f64::INFINITY, |a, b| a.min(b));
        let p_ref_max = p_ref
            .iter()
            .copied()
            .fold(f64::NEG_INFINITY, |a, b| a.max(b));
        let p_sol_min = p_sol.iter().copied().fold(f64::INFINITY, |a, b| a.min(b));
        let p_sol_max = p_sol
            .iter()
            .copied()
            .fold(f64::NEG_INFINITY, |a, b| a.max(b));
