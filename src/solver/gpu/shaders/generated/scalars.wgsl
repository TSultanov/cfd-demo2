...
@group(0) @binding(1) 
var<storage, read> dot_result_1: array<f32>;

@group(0) @binding(2) 
var<storage, read> dot_result_2: array<f32>;
...
fn reduce_rho_new_r_r(@builtin(local_invocation_id) local_id: vec3<u32>) {
    ...
    if (lid == 0u) {
        scalars.rho_new = scratch1[0u];
        scalars.r_r = scratch2[0u];
    }
}
