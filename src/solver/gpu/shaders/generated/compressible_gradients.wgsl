// GENERATED BY CFD2 CODEGEN (compressible_gradients)
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    var grad_rho_accum = Vector2(0.0, 0.0);
    var grad_rho_u_x_accum = Vector2(0.0, 0.0);
    var grad_rho_u_y_accum = Vector2(0.0, 0.0);
    var grad_rho_e_accum = Vector2(0.0, 0.0);
    for (var k = start; k < end; k++) {
        ...
        var rho_face = rho_l;
        var rho_u_face_x = rho_u_l.x;
        var rho_u_face_y = rho_u_l.y;
        var rho_e_face = rho_e_l;
        if (neighbor != -1) {
            rho_face = 0.5 * (rho_l + rho_r);
            rho_u_face_x = 0.5 * (rho_u_l.x + rho_u_r.x);
            rho_u_face_y = 0.5 * (rho_u_l.y + rho_u_r.y);
            rho_e_face = 0.5 * (rho_e_l + rho_e_r);
        }
        grad_rho_accum.x += rho_face * normal.x * area;
        grad_rho_accum.y += rho_face * normal.y * area;
        grad_rho_u_x_accum.x += rho_u_face_x * normal.x * area;
        grad_rho_u_x_accum.y += rho_u_face_x * normal.y * area;
        grad_rho_u_y_accum.x += rho_u_face_y * normal.x * area;
        grad_rho_u_y_accum.y += rho_u_face_y * normal.y * area;
        grad_rho_e_accum.x += rho_e_face * normal.x * area;
        grad_rho_e_accum.y += rho_e_face * normal.y * area;
    }
    grad_rho_accum.x = grad_rho_accum.x / vol;
    grad_rho_accum.y = grad_rho_accum.y / vol;
    grad_rho_u_x_accum.x = grad_rho_u_x_accum.x / vol;
    grad_rho_u_x_accum.y = grad_rho_u_x_accum.y / vol;
    grad_rho_u_y_accum.x = grad_rho_u_y_accum.x / vol;
    grad_rho_u_y_accum.y = grad_rho_u_y_accum.y / vol;
    grad_rho_e_accum.x = grad_rho_e_accum.x / vol;
    grad_rho_e_accum.y = grad_rho_e_accum.y / vol;
    grad_rho[idx] = grad_rho_accum;
    grad_rho_u_x[idx] = grad_rho_u_x_accum;
    grad_rho_u_y[idx] = grad_rho_u_y_accum;
    grad_rho_e[idx] = grad_rho_e_accum;
}
