...
// SourceHash: a84f8d758c4d6c7a1712f828b757a256f7ca9fe51bec8732d6f9e8c2279a76a5
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    loop {
        ...
        }
        {
            ...
            let _e844 = is_boundary;
            let _e848 = constants.precond_model;
            let _e854 = constants.pressure_coupling_alpha;
            if ((!(_e844) && (_e848 != 2u)) && (_e854 > 0f)) {
                ...
                let _e893 = other_idx;
                let grad_rho_r_1 = grad_rho[_e893];
                let _e897 = other_idx;
                let grad_rho_u_x_r_1 = grad_rho_u_x[_e897];
                let _e901 = other_idx;
                let grad_rho_u_y_r_1 = grad_rho_u_y[_e901];
                let _e905 = other_idx;
                let grad_rho_e_r_1 = grad_rho_e[_e905];
                ...
                let _e1017 = normal.x;
                let _e1020 = normal.y;
                let grad_p_l_n = ((grad_p_l_x * _e1017) + (grad_p_l_y * _e1020));
                let _e1024 = normal.x;
                let _e1027 = normal.y;
                let grad_p_r_n = ((grad_p_r_x * _e1024) + (grad_p_r_y * _e1027));
                let grad_p_face_n = (0.5f * (grad_p_l_n + grad_p_r_n));
                let grad_p_jump_n = ((p_r - p_l) / dist);
                let _e1035 = rho_l;
                let _e1036 = rho_r;
                let rho_face = (0.5f * (_e1035 + _e1036));
                let p_bar = (0.5f * (p_l + p_r));
                let dp_rel = (abs((p_r - p_l)) / max(p_bar, 0.000001f));
                let _e1050 = constants.precond_theta_floor;
                let pc_theta = min(1f, max(mach2_, _e1050));
                let pc_low_mach = (1f - pc_theta);
                let pc_smooth = (1f / (1f + (((dp_rel / 0.2f) * dp_rel) / 0.2f)));
                let _e1067 = constants.pressure_coupling_alpha;
                let pc_alpha = ((_e1067 * pc_low_mach) * pc_smooth);
                let _e1072 = constants.dt;
                let m_corr = (((pc_alpha * _e1072) / max(rho_face, 0.00000001f)) * (grad_p_face_n - grad_p_jump_n));
                let _e1079 = rho_e_l;
                let h_l = ((_e1079 + p_l) * du_ly_drv);
                let _e1082 = rho_e_r;
                let h_r = ((_e1082 + p_r) * du_ry_drv);
                let h_face = (0.5f * (h_l + h_r));
                let _e1088 = flux_rho;
                flux_rho = (_e1088 + m_corr);
                let _e1091 = flux_rho_u_x;
                flux_rho_u_x = (_e1091 + (m_corr * u_face_x));
                let _e1094 = flux_rho_u_y;
                flux_rho_u_y = (_e1094 + (m_corr * u_face_y));
                let _e1097 = flux_rho_e;
                flux_rho_e = (_e1097 + (m_corr * h_face));
            }
            let _e1099 = flux_rho_e;
            flux_rho_e = ((_e1099 + (diff_u_x * u_face_x)) + (diff_u_y * u_face_y));
            let _e1105 = flux_rho;
            let _e1107 = sum_rho;
            sum_rho = (_e1107 + (_e1105 * area));
            let _e1110 = flux_rho_u_x;
            let _e1112 = sum_rho_u_x;
            sum_rho_u_x = (_e1112 + (_e1110 * area));
            let _e1115 = flux_rho_u_y;
            let _e1117 = sum_rho_u_y;
            sum_rho_u_y = (_e1117 + (_e1115 * area));
            let _e1120 = flux_rho_e;
            let _e1122 = sum_rho_e;
            sum_rho_e = (_e1122 + (_e1120 * area));
            let q_l = ((u_l_x * u_l_x) + (u_l_y * u_l_y));
            let dp_drho_l = (0.2f * q_l);
            let dp_dru_l = (-0.4f * u_l_x);
            let dp_drv_l = (-0.4f * u_l_y);
            let _e1133 = rho_e_l;
            let H_l = ((_e1133 + p_l) * du_ly_drv);
            let q_r = ((u_r_x * u_r_x) + (u_r_y * u_r_y));
            let dp_drho_r = (0.2f * q_r);
            let dp_dru_r = (-0.4f * u_r_x);
            let dp_drv_r = (-0.4f * u_r_y);
            let _e1145 = rho_e_r;
            let H_r = ((_e1145 + p_r) * du_ry_drv);
            let a_l = (a_plus / denom);
            let a_r = (-(a_minus) / denom);
            jac_l_00_ = -(a_prod_scaled);
            let _e1154 = normal.x;
            jac_l_01_ = (a_l * _e1154);
            let _e1158 = normal.y;
            jac_l_02_ = (a_l * _e1158);
            jac_l_03_ = 0f;
            jac_r_00_ = a_prod_scaled;
            let _e1165 = normal.x;
            jac_r_01_ = (a_r * _e1165);
            let _e1169 = normal.y;
            jac_r_02_ = (a_r * _e1169);
            jac_r_03_ = 0f;
            let _e1177 = normal.x;
            let A_l_10_ = ((-(u_l_x) * u_n_l) + (dp_drho_l * _e1177));
            let _e1181 = normal.x;
            let _e1185 = normal.x;
            let A_l_11_ = ((u_n_l + (u_l_x * _e1181)) + (dp_dru_l * _e1185));
            let _e1189 = normal.y;
            let _e1192 = normal.x;
            let A_l_12_ = ((u_l_x * _e1189) + (dp_drv_l * _e1192));
            let _e1197 = normal.x;
            let A_l_13_ = (0.4f * _e1197);
            let _e1202 = normal.y;
            let A_l_20_ = ((-(u_l_y) * u_n_l) + (dp_drho_l * _e1202));
            let _e1206 = normal.x;
            let _e1209 = normal.y;
            let A_l_21_ = ((u_l_y * _e1206) + (dp_dru_l * _e1209));
            let _e1213 = normal.y;
            let _e1217 = normal.y;
            let A_l_22_ = ((u_n_l + (u_l_y * _e1213)) + (dp_drv_l * _e1217));
            let _e1221 = normal.y;
            let A_l_23_ = (0.4f * _e1221);
            let A_l_30_ = ((-(H_l) * u_n_l) + (dp_drho_l * u_n_l));
            let _e1228 = normal.x;
            let A_l_31_ = ((H_l * _e1228) + (dp_dru_l * u_n_l));
            let _e1233 = normal.y;
            let A_l_32_ = ((H_l * _e1233) + (dp_drv_l * u_n_l));
            let A_l_33_ = (1.4f * u_n_l);
            let _e1242 = normal.x;
            let A_r_10_ = ((-(u_r_x) * u_n_r) + (dp_drho_r * _e1242));
            let _e1246 = normal.x;
            let _e1250 = normal.x;
            let A_r_11_ = ((u_n_r + (u_r_x * _e1246)) + (dp_dru_r * _e1250));
            let _e1254 = normal.y;
            let _e1257 = normal.x;
            let A_r_12_ = ((u_r_x * _e1254) + (dp_drv_r * _e1257));
            let _e1262 = normal.x;
            let A_r_13_ = (0.4f * _e1262);
            let _e1267 = normal.y;
            let A_r_20_ = ((-(u_r_y) * u_n_r) + (dp_drho_r * _e1267));
            let _e1271 = normal.x;
            let _e1274 = normal.y;
            let A_r_21_ = ((u_r_y * _e1271) + (dp_dru_r * _e1274));
            let _e1278 = normal.y;
            let _e1282 = normal.y;
            let A_r_22_ = ((u_n_r + (u_r_y * _e1278)) + (dp_drv_r * _e1282));
            let _e1286 = normal.y;
            let A_r_23_ = (0.4f * _e1286);
            let A_r_30_ = ((-(H_r) * u_n_r) + (dp_drho_r * u_n_r));
            let _e1293 = normal.x;
            let A_r_31_ = ((H_r * _e1293) + (dp_dru_r * u_n_r));
            let _e1298 = normal.y;
            let A_r_32_ = ((H_r * _e1298) + (dp_drv_r * u_n_r));
            ...
            let _e1379 = jac_l_10_;
            jac_l_10_ = (_e1379 + d_diff_x_l_rho);
            let _e1381 = jac_l_11_;
            jac_l_11_ = (_e1381 + d_diff_x_l_ru);
            let _e1384 = jac_l_12_;
            jac_l_12_ = (_e1384 + 0f);
            let _e1387 = jac_l_13_;
            jac_l_13_ = (_e1387 + 0f);
            let _e1389 = jac_l_20_;
            jac_l_20_ = (_e1389 + d_diff_y_l_rho);
            let _e1392 = jac_l_21_;
            jac_l_21_ = (_e1392 + 0f);
            let _e1394 = jac_l_22_;
            jac_l_22_ = (_e1394 + d_diff_y_l_rv);
            let _e1397 = jac_l_23_;
            jac_l_23_ = (_e1397 + 0f);
            let _e1399 = jac_r_10_;
            jac_r_10_ = (_e1399 + d_diff_x_r_rho);
            let _e1401 = jac_r_11_;
            jac_r_11_ = (_e1401 + d_diff_x_r_ru);
            let _e1404 = jac_r_12_;
            jac_r_12_ = (_e1404 + 0f);
            let _e1407 = jac_r_13_;
            jac_r_13_ = (_e1407 + 0f);
            let _e1409 = jac_r_20_;
            jac_r_20_ = (_e1409 + d_diff_y_r_rho);
            let _e1412 = jac_r_21_;
            jac_r_21_ = (_e1412 + 0f);
            let _e1414 = jac_r_22_;
            jac_r_22_ = (_e1414 + d_diff_y_r_rv);
            let _e1417 = jac_r_23_;
            jac_r_23_ = (_e1417 + 0f);
            ...
            let _e1499 = jac_l_30_;
            jac_l_30_ = (_e1499 + d_e_visc_l_rho);
            let _e1501 = jac_l_31_;
            jac_l_31_ = (_e1501 + d_e_visc_l_ru);
            let _e1503 = jac_l_32_;
            jac_l_32_ = (_e1503 + d_e_visc_l_rv);
            let _e1505 = jac_l_33_;
            jac_l_33_ = (_e1505 + d_e_visc_l_re);
            let _e1507 = jac_r_30_;
            jac_r_30_ = (_e1507 + d_e_visc_r_rho);
            let _e1509 = jac_r_31_;
            jac_r_31_ = (_e1509 + d_e_visc_r_ru);
            let _e1511 = jac_r_32_;
            jac_r_32_ = (_e1511 + d_e_visc_r_rv);
            let _e1513 = jac_r_33_;
            jac_r_33_ = (_e1513 + d_e_visc_r_re);
            let _e1515 = is_boundary;
            if !(_e1515) {
                let _e1518 = face_offset;
                let scalar_mat_idx = cell_face_matrix_indices[_e1518];
                neighbor_rank = 0u;
                ...
                let _e1528 = neighbor_rank;
                let _e1535 = jac_r_00_;
                matrix_values[((start_row_0_ + (4u * _e1528)) + 0u)] = (_e1535 * area);
                let _e1538 = neighbor_rank;
                let _e1545 = jac_r_01_;
                matrix_values[((start_row_0_ + (4u * _e1538)) + 1u)] = (_e1545 * area);
                let _e1548 = neighbor_rank;
                let _e1555 = jac_r_02_;
                matrix_values[((start_row_0_ + (4u * _e1548)) + 2u)] = (_e1555 * area);
                let _e1558 = neighbor_rank;
                let _e1565 = jac_r_03_;
                matrix_values[((start_row_0_ + (4u * _e1558)) + 3u)] = (_e1565 * area);
                let _e1568 = neighbor_rank;
                let _e1575 = jac_r_10_;
                matrix_values[((start_row_1_ + (4u * _e1568)) + 0u)] = (_e1575 * area);
                let _e1578 = neighbor_rank;
                let _e1585 = jac_r_11_;
                matrix_values[((start_row_1_ + (4u * _e1578)) + 1u)] = (_e1585 * area);
                let _e1588 = neighbor_rank;
                let _e1595 = jac_r_12_;
                matrix_values[((start_row_1_ + (4u * _e1588)) + 2u)] = (_e1595 * area);
                let _e1598 = neighbor_rank;
                let _e1605 = jac_r_13_;
                matrix_values[((start_row_1_ + (4u * _e1598)) + 3u)] = (_e1605 * area);
                let _e1608 = neighbor_rank;
                let _e1615 = jac_r_20_;
                matrix_values[((start_row_2_ + (4u * _e1608)) + 0u)] = (_e1615 * area);
                let _e1618 = neighbor_rank;
                let _e1625 = jac_r_21_;
                matrix_values[((start_row_2_ + (4u * _e1618)) + 1u)] = (_e1625 * area);
                let _e1628 = neighbor_rank;
                let _e1635 = jac_r_22_;
                matrix_values[((start_row_2_ + (4u * _e1628)) + 2u)] = (_e1635 * area);
                let _e1638 = neighbor_rank;
                let _e1645 = jac_r_23_;
                matrix_values[((start_row_2_ + (4u * _e1638)) + 3u)] = (_e1645 * area);
                let _e1648 = neighbor_rank;
                let _e1655 = jac_r_30_;
                matrix_values[((start_row_3_ + (4u * _e1648)) + 0u)] = (_e1655 * area);
                let _e1658 = neighbor_rank;
                let _e1665 = jac_r_31_;
                matrix_values[((start_row_3_ + (4u * _e1658)) + 1u)] = (_e1665 * area);
                let _e1668 = neighbor_rank;
                let _e1675 = jac_r_32_;
                matrix_values[((start_row_3_ + (4u * _e1668)) + 2u)] = (_e1675 * area);
                let _e1678 = neighbor_rank;
                let _e1685 = jac_r_33_;
                matrix_values[((start_row_3_ + (4u * _e1678)) + 3u)] = (_e1685 * area);
                let _e1688 = jac_l_00_;
                let _e1690 = diag_00_;
                diag_00_ = (_e1690 + (_e1688 * area));
                let _e1693 = jac_l_01_;
                let _e1695 = diag_01_;
                diag_01_ = (_e1695 + (_e1693 * area));
                let _e1698 = jac_l_02_;
                let _e1700 = diag_02_;
                diag_02_ = (_e1700 + (_e1698 * area));
                let _e1703 = jac_l_03_;
                let _e1705 = diag_03_;
                diag_03_ = (_e1705 + (_e1703 * area));
                let _e1708 = jac_l_10_;
                let _e1710 = diag_10_;
                diag_10_ = (_e1710 + (_e1708 * area));
                let _e1713 = jac_l_11_;
                let _e1715 = diag_11_;
                diag_11_ = (_e1715 + (_e1713 * area));
                let _e1718 = jac_l_12_;
                let _e1720 = diag_12_;
                diag_12_ = (_e1720 + (_e1718 * area));
                let _e1723 = jac_l_13_;
                let _e1725 = diag_13_;
                diag_13_ = (_e1725 + (_e1723 * area));
                let _e1728 = jac_l_20_;
                let _e1730 = diag_20_;
                diag_20_ = (_e1730 + (_e1728 * area));
                let _e1733 = jac_l_21_;
                let _e1735 = diag_21_;
                diag_21_ = (_e1735 + (_e1733 * area));
                let _e1738 = jac_l_22_;
                let _e1740 = diag_22_;
                diag_22_ = (_e1740 + (_e1738 * area));
                let _e1743 = jac_l_23_;
                let _e1745 = diag_23_;
                diag_23_ = (_e1745 + (_e1743 * area));
                let _e1748 = jac_l_30_;
                let _e1750 = diag_30_;
                diag_30_ = (_e1750 + (_e1748 * area));
                let _e1753 = jac_l_31_;
                let _e1755 = diag_31_;
                diag_31_ = (_e1755 + (_e1753 * area));
                let _e1758 = jac_l_32_;
                let _e1760 = diag_32_;
                diag_32_ = (_e1760 + (_e1758 * area));
                let _e1763 = jac_l_33_;
                let _e1765 = diag_33_;
                diag_33_ = (_e1765 + (_e1763 * area));
            } else {
                if (boundary_type == 1u) {
                    let _e1769 = jac_l_00_;
                    let _e1770 = jac_r_00_;
                    let _e1772 = jac_r_01_;
                    let _e1775 = constants.inlet_velocity;
                    let _e1779 = diag_00_;
                    diag_00_ = (_e1779 + (((_e1769 + _e1770) + (_e1772 * _e1775)) * area));
                    let _e1781 = jac_l_01_;
                    let _e1783 = diag_01_;
                    diag_01_ = (_e1783 + (_e1781 * area));
                    let _e1785 = jac_l_02_;
                    let _e1787 = diag_02_;
                    diag_02_ = (_e1787 + (_e1785 * area));
                    let _e1789 = jac_l_03_;
                    let _e1790 = jac_r_03_;
                    let _e1793 = diag_03_;
                    diag_03_ = (_e1793 + ((_e1789 + _e1790) * area));
                    let _e1795 = jac_l_10_;
                    let _e1796 = jac_r_10_;
                    let _e1798 = jac_r_11_;
                    let _e1801 = constants.inlet_velocity;
                    let _e1805 = diag_10_;
                    diag_10_ = (_e1805 + (((_e1795 + _e1796) + (_e1798 * _e1801)) * area));
                    let _e1807 = jac_l_11_;
                    let _e1809 = diag_11_;
                    diag_11_ = (_e1809 + (_e1807 * area));
                    let _e1811 = jac_l_12_;
                    let _e1813 = diag_12_;
                    diag_12_ = (_e1813 + (_e1811 * area));
                    let _e1815 = jac_l_13_;
                    let _e1816 = jac_r_13_;
                    let _e1819 = diag_13_;
                    diag_13_ = (_e1819 + ((_e1815 + _e1816) * area));
                    let _e1821 = jac_l_20_;
                    let _e1822 = jac_r_20_;
                    let _e1824 = jac_r_21_;
                    let _e1827 = constants.inlet_velocity;
                    let _e1831 = diag_20_;
                    diag_20_ = (_e1831 + (((_e1821 + _e1822) + (_e1824 * _e1827)) * area));
                    let _e1833 = jac_l_21_;
                    let _e1835 = diag_21_;
                    diag_21_ = (_e1835 + (_e1833 * area));
                    let _e1837 = jac_l_22_;
                    let _e1839 = diag_22_;
                    diag_22_ = (_e1839 + (_e1837 * area));
                    let _e1841 = jac_l_23_;
                    let _e1842 = jac_r_23_;
                    let _e1845 = diag_23_;
                    diag_23_ = (_e1845 + ((_e1841 + _e1842) * area));
                    let _e1847 = jac_l_30_;
                    let _e1848 = jac_r_30_;
                    let _e1850 = jac_r_31_;
                    let _e1853 = constants.inlet_velocity;
                    let _e1857 = diag_30_;
                    diag_30_ = (_e1857 + (((_e1847 + _e1848) + (_e1850 * _e1853)) * area));
                    let _e1859 = jac_l_31_;
                    let _e1861 = diag_31_;
                    diag_31_ = (_e1861 + (_e1859 * area));
                    let _e1863 = jac_l_32_;
                    let _e1865 = diag_32_;
                    diag_32_ = (_e1865 + (_e1863 * area));
                    let _e1867 = jac_l_33_;
                    let _e1868 = jac_r_33_;
                    let _e1871 = diag_33_;
                    diag_33_ = (_e1871 + ((_e1867 + _e1868) * area));
                } else {
                    if (boundary_type == 3u) {
                        let _e1876 = normal.x;
                        let _e1880 = normal.x;
                        let r11_ = (1f - ((2f * _e1876) * _e1880));
                        let _e1885 = normal.x;
                        let _e1889 = normal.y;
                        let r12_ = ((-2f * _e1885) * _e1889);
                        let _e1892 = normal.y;
                        let _e1896 = normal.x;
                        let r21_ = ((-2f * _e1892) * _e1896);
                        let _e1899 = normal.y;
                        let _e1903 = normal.y;
                        let r22_ = (1f - ((2f * _e1899) * _e1903));
                        let _e1907 = jac_l_00_;
                        let _e1908 = jac_r_00_;
                        let _e1911 = diag_00_;
                        diag_00_ = (_e1911 + ((_e1907 + _e1908) * area));
                        let _e1913 = jac_l_01_;
                        let _e1914 = jac_r_01_;
                        let _e1917 = jac_r_02_;
                        let _e1921 = diag_01_;
                        diag_01_ = (_e1921 + (((_e1913 + (_e1914 * r11_)) + (_e1917 * r21_)) * area));
                        let _e1923 = jac_l_02_;
                        let _e1924 = jac_r_01_;
                        let _e1927 = jac_r_02_;
                        let _e1931 = diag_02_;
                        diag_02_ = (_e1931 + (((_e1923 + (_e1924 * r12_)) + (_e1927 * r22_)) * area));
                        let _e1933 = jac_l_03_;
                        let _e1934 = jac_r_03_;
                        let _e1937 = diag_03_;
                        diag_03_ = (_e1937 + ((_e1933 + _e1934) * area));
                        let _e1939 = jac_l_10_;
                        let _e1940 = jac_r_10_;
                        let _e1943 = diag_10_;
                        diag_10_ = (_e1943 + ((_e1939 + _e1940) * area));
                        let _e1945 = jac_l_11_;
                        let _e1946 = jac_r_11_;
                        let _e1949 = jac_r_12_;
                        let _e1953 = diag_11_;
                        diag_11_ = (_e1953 + (((_e1945 + (_e1946 * r11_)) + (_e1949 * r21_)) * area));
                        let _e1955 = jac_l_12_;
                        let _e1956 = jac_r_11_;
                        let _e1959 = jac_r_12_;
                        let _e1963 = diag_12_;
                        diag_12_ = (_e1963 + (((_e1955 + (_e1956 * r12_)) + (_e1959 * r22_)) * area));
                        let _e1965 = jac_l_13_;
                        let _e1966 = jac_r_13_;
                        let _e1969 = diag_13_;
                        diag_13_ = (_e1969 + ((_e1965 + _e1966) * area));
                        let _e1971 = jac_l_20_;
                        let _e1972 = jac_r_20_;
                        let _e1975 = diag_20_;
                        diag_20_ = (_e1975 + ((_e1971 + _e1972) * area));
                        let _e1977 = jac_l_21_;
                        let _e1978 = jac_r_21_;
                        let _e1981 = jac_r_22_;
                        let _e1985 = diag_21_;
                        diag_21_ = (_e1985 + (((_e1977 + (_e1978 * r11_)) + (_e1981 * r21_)) * area));
                        let _e1987 = jac_l_22_;
                        let _e1988 = jac_r_21_;
                        let _e1991 = jac_r_22_;
                        let _e1995 = diag_22_;
                        diag_22_ = (_e1995 + (((_e1987 + (_e1988 * r12_)) + (_e1991 * r22_)) * area));
                        let _e1997 = jac_l_23_;
                        let _e1998 = jac_r_23_;
                        let _e2001 = diag_23_;
                        diag_23_ = (_e2001 + ((_e1997 + _e1998) * area));
                        let _e2003 = jac_l_30_;
                        let _e2004 = jac_r_30_;
                        let _e2007 = diag_30_;
                        diag_30_ = (_e2007 + ((_e2003 + _e2004) * area));
                        let _e2009 = jac_l_31_;
                        let _e2010 = jac_r_31_;
                        let _e2013 = jac_r_32_;
                        let _e2017 = diag_31_;
                        diag_31_ = (_e2017 + (((_e2009 + (_e2010 * r11_)) + (_e2013 * r21_)) * area));
                        let _e2019 = jac_l_32_;
                        let _e2020 = jac_r_31_;
                        let _e2023 = jac_r_32_;
                        let _e2027 = diag_32_;
                        diag_32_ = (_e2027 + (((_e2019 + (_e2020 * r12_)) + (_e2023 * r22_)) * area));
                        let _e2029 = jac_l_33_;
                        let _e2030 = jac_r_33_;
                        let _e2033 = diag_33_;
                        diag_33_ = (_e2033 + ((_e2029 + _e2030) * area));
                    } else {
                        let _e2035 = jac_l_00_;
                        let _e2036 = jac_r_00_;
                        let _e2039 = diag_00_;
                        diag_00_ = (_e2039 + ((_e2035 + _e2036) * area));
                        let _e2041 = jac_l_01_;
                        let _e2042 = jac_r_01_;
                        let _e2045 = diag_01_;
                        diag_01_ = (_e2045 + ((_e2041 + _e2042) * area));
                        let _e2047 = jac_l_02_;
                        let _e2048 = jac_r_02_;
                        let _e2051 = diag_02_;
                        diag_02_ = (_e2051 + ((_e2047 + _e2048) * area));
                        let _e2053 = jac_l_03_;
                        let _e2054 = jac_r_03_;
                        let _e2057 = diag_03_;
                        diag_03_ = (_e2057 + ((_e2053 + _e2054) * area));
                        let _e2059 = jac_l_10_;
                        let _e2060 = jac_r_10_;
                        let _e2063 = diag_10_;
                        diag_10_ = (_e2063 + ((_e2059 + _e2060) * area));
                        let _e2065 = jac_l_11_;
                        let _e2066 = jac_r_11_;
                        let _e2069 = diag_11_;
                        diag_11_ = (_e2069 + ((_e2065 + _e2066) * area));
                        let _e2071 = jac_l_12_;
                        let _e2072 = jac_r_12_;
                        let _e2075 = diag_12_;
                        diag_12_ = (_e2075 + ((_e2071 + _e2072) * area));
                        let _e2077 = jac_l_13_;
                        let _e2078 = jac_r_13_;
                        let _e2081 = diag_13_;
                        diag_13_ = (_e2081 + ((_e2077 + _e2078) * area));
                        let _e2083 = jac_l_20_;
                        let _e2084 = jac_r_20_;
                        let _e2087 = diag_20_;
                        diag_20_ = (_e2087 + ((_e2083 + _e2084) * area));
                        let _e2089 = jac_l_21_;
                        let _e2090 = jac_r_21_;
                        let _e2093 = diag_21_;
                        diag_21_ = (_e2093 + ((_e2089 + _e2090) * area));
                        let _e2095 = jac_l_22_;
                        let _e2096 = jac_r_22_;
                        let _e2099 = diag_22_;
                        diag_22_ = (_e2099 + ((_e2095 + _e2096) * area));
                        let _e2101 = jac_l_23_;
                        let _e2102 = jac_r_23_;
                        let _e2105 = diag_23_;
                        diag_23_ = (_e2105 + ((_e2101 + _e2102) * area));
                        let _e2107 = jac_l_30_;
                        let _e2108 = jac_r_30_;
                        let _e2111 = diag_30_;
                        diag_30_ = (_e2111 + ((_e2107 + _e2108) * area));
                        let _e2113 = jac_l_31_;
                        let _e2114 = jac_r_31_;
                        let _e2117 = diag_31_;
                        diag_31_ = (_e2117 + ((_e2113 + _e2114) * area));
                        let _e2119 = jac_l_32_;
                        let _e2120 = jac_r_32_;
                        let _e2123 = diag_32_;
                        diag_32_ = (_e2123 + ((_e2119 + _e2120) * area));
                        let _e2125 = jac_l_33_;
                        let _e2126 = jac_r_33_;
                        let _e2129 = diag_33_;
                        diag_33_ = (_e2129 + ((_e2125 + _e2126) * area));
                    }
                }
            }
        }
        continuing {
            let _e2132 = face_offset;
            face_offset = (_e2132 + 1u);
        }
    }
    let _e2134 = rhs_time_rho;
    let _e2135 = rhs_pseudo_rho;
    let _e2137 = coeff_time;
    let _e2138 = coeff_pseudo;
    let _e2142 = sum_rho;
    rhs_rho = (((_e2134 + _e2135) - ((_e2137 + _e2138) * rho)) - _e2142);
    let _e2145 = rhs_time_rho_u_x;
    let _e2146 = rhs_pseudo_rho_u_x;
    let _e2148 = coeff_time;
    let _e2149 = coeff_pseudo;
    let _e2154 = sum_rho_u_x;
    rhs_rho_u_x = (((_e2145 + _e2146) - ((_e2148 + _e2149) * rho_u.x)) - _e2154);
    let _e2157 = rhs_time_rho_u_y;
    let _e2158 = rhs_pseudo_rho_u_y;
    let _e2160 = coeff_time;
    let _e2161 = coeff_pseudo;
    let _e2166 = sum_rho_u_y;
    rhs_rho_u_y = (((_e2157 + _e2158) - ((_e2160 + _e2161) * rho_u.y)) - _e2166);
    let _e2169 = rhs_time_rho_e;
    let _e2170 = rhs_pseudo_rho_e;
    let _e2172 = coeff_time;
    let _e2173 = coeff_pseudo;
    let _e2177 = sum_rho_e;
    rhs_rho_e = (((_e2169 + _e2170) - ((_e2172 + _e2173) * rho_e)) - _e2177);
    let _e2180 = coeff_time;
    let _e2181 = diag_00_;
    diag_00_ = (_e2181 + _e2180);
    let _e2183 = coeff_time;
    let _e2184 = diag_11_;
    diag_11_ = (_e2184 + _e2183);
    let _e2186 = coeff_time;
    let _e2187 = diag_22_;
    diag_22_ = (_e2187 + _e2186);
    let _e2189 = coeff_time;
    let _e2190 = diag_33_;
    diag_33_ = (_e2190 + _e2189);
    let _e2192 = coeff_pseudo;
    let _e2193 = diag_00_;
    diag_00_ = (_e2193 + _e2192);
    let _e2195 = coeff_pseudo;
    let _e2196 = diag_11_;
    diag_11_ = (_e2196 + _e2195);
    let _e2198 = coeff_pseudo;
    let _e2199 = diag_22_;
    diag_22_ = (_e2199 + _e2198);
    let _e2201 = coeff_pseudo;
    let _e2202 = diag_33_;
    diag_33_ = (_e2202 + _e2201);
    let scalar_diag_idx = diagonal_indices[idx];
    let diag_rank = (scalar_diag_idx - scalar_offset);
    let _e2215 = diag_00_;
    matrix_values[((start_row_0_ + (4u * diag_rank)) + 0u)] = _e2215;
    let _e2223 = diag_01_;
    matrix_values[((start_row_0_ + (4u * diag_rank)) + 1u)] = _e2223;
    let _e2231 = diag_02_;
    matrix_values[((start_row_0_ + (4u * diag_rank)) + 2u)] = _e2231;
    let _e2239 = diag_03_;
    matrix_values[((start_row_0_ + (4u * diag_rank)) + 3u)] = _e2239;
    let _e2247 = diag_10_;
    matrix_values[((start_row_1_ + (4u * diag_rank)) + 0u)] = _e2247;
    let _e2255 = diag_11_;
    matrix_values[((start_row_1_ + (4u * diag_rank)) + 1u)] = _e2255;
    let _e2263 = diag_12_;
    matrix_values[((start_row_1_ + (4u * diag_rank)) + 2u)] = _e2263;
    let _e2271 = diag_13_;
    matrix_values[((start_row_1_ + (4u * diag_rank)) + 3u)] = _e2271;
    let _e2279 = diag_20_;
    matrix_values[((start_row_2_ + (4u * diag_rank)) + 0u)] = _e2279;
    let _e2287 = diag_21_;
    matrix_values[((start_row_2_ + (4u * diag_rank)) + 1u)] = _e2287;
    let _e2295 = diag_22_;
    matrix_values[((start_row_2_ + (4u * diag_rank)) + 2u)] = _e2295;
    let _e2303 = diag_23_;
    matrix_values[((start_row_2_ + (4u * diag_rank)) + 3u)] = _e2303;
    let _e2311 = diag_30_;
    matrix_values[((start_row_3_ + (4u * diag_rank)) + 0u)] = _e2311;
    let _e2319 = diag_31_;
    matrix_values[((start_row_3_ + (4u * diag_rank)) + 1u)] = _e2319;
    let _e2327 = diag_32_;
    matrix_values[((start_row_3_ + (4u * diag_rank)) + 2u)] = _e2327;
    let _e2335 = diag_33_;
    matrix_values[((start_row_3_ + (4u * diag_rank)) + 3u)] = _e2335;
    let _e2342 = rhs_rho;
    rhs[((idx * 4u) + 0u)] = _e2342;
    let _e2349 = rhs_rho_u_x;
    rhs[((idx * 4u) + 1u)] = _e2349;
    let _e2356 = rhs_rho_u_y;
    rhs[((idx * 4u) + 2u)] = _e2356;
    let _e2363 = rhs_rho_e;
    rhs[((idx * 4u) + 3u)] = _e2363;
    return;
}
"#;
    }
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    let _e777 = constants.precond_model;
    let _e783 = constants.pressure_coupling_alpha;
    if ((!(_e773) && (_e777 != 2u)) && (_e783 > 0f)) {
        let _e789 = constants.precond_theta_floor;
        let pc_theta = min(1f, max(mach2_, _e789));
        let pc_low_mach = (1f - pc_theta);
        let pc_smooth = (1f / (1f + (((dp_rel / 0.2f) * dp_rel) / 0.2f)));
        let _e806 = constants.pressure_coupling_alpha;
        let pc_alpha = ((_e806 * pc_low_mach) * pc_smooth);
        let _e811 = constants.dt;
        let m_corr = (((pc_alpha * _e811) / max(rho_face, 0.00000001f)) * (grad_p_face_n - grad_p_jump_n));
        let _e818 = rho_e_l;
        let h_l = ((_e818 + p_l) * inv_rho_l);
        let _e821 = rho_e_r;
        let h_r = ((_e821 + p_r) * inv_rho_r);
        let h_face = (0.5f * (h_l + h_r));
        let _e827 = flux_rho;
        flux_rho = (_e827 + m_corr);
        let _e829 = flux_rho_u_x;
        flux_rho_u_x = (_e829 + (m_corr * u_face_x));
        let _e832 = flux_rho_u_y;
        flux_rho_u_y = (_e832 + (m_corr * u_face_y));
        let _e835 = flux_rho_e;
        flux_rho_e = (_e835 + (m_corr * h_face));
    }
    let _e838 = flux_rho_e;
    flux_rho_e = ((_e838 + (diff_u_x * u_face_x)) + (diff_u_y * u_face_y));
    let base = (idx * 4u);
    let _e849 = flux_rho;
    fluxes[(base + 0u)] = (_e849 * area);
    let _e855 = flux_rho_u_x;
    fluxes[(base + 1u)] = (_e855 * area);
    let _e861 = flux_rho_u_y;
    fluxes[(base + 2u)] = (_e861 * area);
    let _e867 = flux_rho_e;
    fluxes[(base + 3u)] = (_e867 * area);
    return;
}
"#;
    }
                ...
                if (0u != 0u) {
                    ...
                    if (0u == 1u) {
                        if (flux_2 > 0f) {
                            ...
                            let _e318 = grad_u[idx].x;
                            let _e324 = grad_u[idx].y;
                            phi_ho_u = ((u_own.x + (_e318 * r_u_x)) + (_e324 * r_u_y));
                        } else {
                            let _e329 = other_center.x;
                            let r_u_x_1 = (f_center.x - _e329);
                            let _e333 = other_center.y;
                            let r_u_y_1 = (f_center.y - _e333);
                            let _e337 = other_idx;
                            let _e340 = grad_u[_e337].x;
                            let _e344 = other_idx;
                            let _e347 = grad_u[_e344].y;
                            phi_ho_u = ((u_neigh.x + (_e340 * r_u_x_1)) + (_e347 * r_u_y_1));
                        }
                    } else {
                        if (0u == 2u) {
                            if (flux_2 > 0f) {
                                let _e355 = other_center.x;
                                let d_cd_u_x = (_e355 - center.x);
                                let _e359 = other_center.y;
                                let d_cd_u_y = (_e359 - center.y);
                                let _e365 = grad_u[idx].x;
                                let _e370 = grad_u[idx].y;
                                let grad_term_u = ((_e365 * d_cd_u_x) + (_e370 * d_cd_u_y));
                                phi_ho_u = (((0.625f * u_own.x) + (0.375f * u_neigh.x)) + (0.125f * grad_term_u));
                            } else {
                                let _e385 = other_center.x;
                                let d_cd_u_x_1 = (center.x - _e385);
                                let _e389 = other_center.y;
                                let d_cd_u_y_1 = (center.y - _e389);
                                let _e392 = other_idx;
                                let _e395 = grad_u[_e392].x;
                                let _e398 = other_idx;
                                let _e401 = grad_u[_e398].y;
                                let grad_term_u_1 = ((_e395 * d_cd_u_x_1) + (_e401 * d_cd_u_y_1));
                    ...
                    if (0u == 1u) {
                        if (flux_2 > 0f) {
                            ...
                            let _e435 = grad_v[idx].x;
                            let _e441 = grad_v[idx].y;
                            phi_ho_v = ((u_own.y + (_e435 * r_v_x)) + (_e441 * r_v_y));
                        } else {
                            let _e446 = other_center.x;
                            let r_v_x_1 = (f_center.x - _e446);
                            let _e450 = other_center.y;
                            let r_v_y_1 = (f_center.y - _e450);
                            let _e454 = other_idx;
                            let _e457 = grad_v[_e454].x;
                            let _e461 = other_idx;
                            let _e464 = grad_v[_e461].y;
                            phi_ho_v = ((u_neigh.y + (_e457 * r_v_x_1)) + (_e464 * r_v_y_1));
                        }
                    } else {
                        if (0u == 2u) {
                            if (flux_2 > 0f) {
                                let _e472 = other_center.x;
                                let d_cd_v_x = (_e472 - center.x);
                                let _e476 = other_center.y;
                                let d_cd_v_y = (_e476 - center.y);
                                let _e482 = grad_v[idx].x;
                                let _e487 = grad_v[idx].y;
                                let grad_term_v = ((_e482 * d_cd_v_x) + (_e487 * d_cd_v_y));
                                phi_ho_v = (((0.625f * u_own.y) + (0.375f * u_neigh.y)) + (0.125f * grad_term_v));
                            } else {
                                let _e502 = other_center.x;
                                let d_cd_v_x_1 = (center.x - _e502);
                                let _e506 = other_center.y;
                                let d_cd_v_y_1 = (center.y - _e506);
                                let _e509 = other_idx;
                                let _e512 = grad_v[_e509].x;
                                let _e515 = other_idx;
                                let _e518 = grad_v[_e515].y;
                                let grad_term_v_1 = ((_e512 * d_cd_v_x_1) + (_e518 * d_cd_v_y_1));
