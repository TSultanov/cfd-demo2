...
fn generate_wgsl_binding_meta(manifest_dir: &str) {
    ...
    for path in list_wgsl_files_recursive(&shader_dir) {
        ...
        let const_name = wgsl_bindings_const_name(&rel_stem);
...
fn generate_kernel_registry_map(
    ...
    let mut id_only_entries: Vec<(String, String, String, String)> = Vec::new();
    for path in list_wgsl_files_recursive(&shader_dir) {
        ...
        let bindings_const = wgsl_bindings_const_name(&rel_stem);
        if entrypoints.len() == 1 && entrypoints[0] == "main" {
            id_only_entries.push((
                rel_stem,
                module_path,
        ...
        for entry in entrypoints {
            id_only_entries.push((
                ...
                bindings_const.clone(),
            ));
        }
    }
    id_only_entries.sort_by(|a, b| a.0.cmp(&b.0).then(a.1.cmp(&b.1)));
    id_only_entries.dedup_by(|a, b| a.0 == b.0);
    ...
    for (kernel_id, module, ctor, bindings_const) in id_only_entries {
...
fn wgsl_bindings_const_name(rel_stem: &str) -> String {
    let ident = rel_stem
        .split('/')
        .filter(|s| !s.is_empty())
        .map(|seg| sanitize_rust_ident(seg).to_ascii_uppercase())
        .collect::<Vec<_>>()
        .join("_");
    format!("{ident}_BINDINGS")
}
