...
pub fn generate_flux_rhie_chow_wgsl(
    ...
    assert!(flux_stride > 0, "flux_rhie_chow requires flux_stride > 0");
...
fn main_body(
    ...
    fn assign_packed_flux(flux_stride: u32, value: Expr) -> Vec<Stmt> {
        let mut stmts = Vec::new();
    ...
    let interior_block = {
        ...
        body.extend(assign_packed_flux(flux_stride, Expr::ident("phi")));
        dsl::block(body)
    };
    ...
    let wall_block = dsl::block(assign_packed_flux(flux_stride, 0.0.into()));
    ...
    let outlet_block = {
        let mut body = vec![
            ...
            dsl::let_expr("phi", dsl::max(0.0, "raw_flux")),
        ];
        body.extend(assign_packed_flux(flux_stride, Expr::ident("phi")));
        dsl::block(body)
    };
    ...
    let boundary_block = dsl::block(vec![dsl::if_block_expr(
        ...
        Some(dsl::block(vec![dsl::if_block_expr(
            ...
            Some(dsl::block(vec![dsl::if_block_expr(
                ...
                outlet_block,
                Some(dsl::block(assign_packed_flux(flux_stride, 0.0.into()))),
            )])),
        )])),
    )]);
...
mod tests {
    ...
    use crate::solver::ir::{fvm, vol_scalar, vol_vector, Coefficient, FluxLayout, SchemeRegistry};
    ...
    fn flux_rhie_chow_codegen_emits_state_arrays() {
        ...
        let flux_stride = FluxLayout::from_system(&system).stride;
        let wgsl =
            generate_flux_rhie_chow_wgsl(&discrete, &layout, flux_stride, "U", "p", "d_p", "grad_p");
