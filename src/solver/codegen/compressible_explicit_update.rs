...
pub fn generate_compressible_explicit_update_wgsl(
    ...
    module.push(Item::Comment(
        "GENERATED BY CFD2 CODEGEN (compressible_explicit_update)".to_string(),
    ));
...
fn main_body(layout: &StateLayout, fields: &CompressibleFields) -> Block {
    ...
    let rho = fields.rho.name();
    let rho_u = fields.rho_u.name();
    let rho_e = fields.rho_e.name();
    ...
    let loop_body = {
        ...
        loop_stmts.push(dsl::assign_op_expr(
            AssignOp::Add,
            Expr::ident("sum_rho"),
            Expr::ident("sign") * Expr::ident("fluxes").index(Expr::ident("base") + 0u32),
        ));
        loop_stmts.push(dsl::assign_op_expr(
            AssignOp::Add,
            Expr::ident("sum_rho_u_x"),
            Expr::ident("sign") * Expr::ident("fluxes").index(Expr::ident("base") + 1u32),
        ));
        loop_stmts.push(dsl::assign_op_expr(
            AssignOp::Add,
            Expr::ident("sum_rho_u_y"),
            Expr::ident("sign") * Expr::ident("fluxes").index(Expr::ident("base") + 2u32),
        ));
        loop_stmts.push(dsl::assign_op_expr(
            AssignOp::Add,
            Expr::ident("sum_rho_e"),
            Expr::ident("sign") * Expr::ident("fluxes").index(Expr::ident("base") + 3u32),
        ));
