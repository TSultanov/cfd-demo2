...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    let s_own_U_x = select(bc_neighbor_scalar(state[owner * 11u + 0u], state[owner * 11u + 0u], bc_kind[idx * 3u + 0u], bc_value[idx * 3u + 0u], d_own, is_boundary), state[owner * 11u + 0u] - (state[owner * 11u + 0u] * normal_vec.x + state[owner * 11u + 1u] * normal_vec.y) * normal_vec.x, is_boundary && boundary_type == 4u);
    let s_own_U_y = select(bc_neighbor_scalar(state[owner * 11u + 1u], state[owner * 11u + 1u], bc_kind[idx * 3u + 1u], bc_value[idx * 3u + 1u], d_own, is_boundary), state[owner * 11u + 1u] - (state[owner * 11u + 0u] * normal_vec.x + state[owner * 11u + 1u] * normal_vec.y) * normal_vec.y, is_boundary && boundary_type == 4u);
    let s_own_d_p = state[owner * 11u + 3u];
    let s_own_grad_p_x = state[owner * 11u + 4u];
    let s_own_grad_p_y = state[owner * 11u + 5u];
    let s_own_p = bc_neighbor_scalar(state[owner * 11u + 2u], state[owner * 11u + 2u], bc_kind[idx * 3u + 2u], bc_value[idx * 3u + 2u], d_own, is_boundary);
    let s_own_rho = state[owner * 11u + 10u];
    let s_neigh_U_x = select(bc_neighbor_scalar(state[neigh_idx * 11u + 0u], state[owner * 11u + 0u], bc_kind[idx * 3u + 0u], bc_value[idx * 3u + 0u], d_own, is_boundary), state[owner * 11u + 0u] - (state[owner * 11u + 0u] * normal_vec.x + state[owner * 11u + 1u] * normal_vec.y) * normal_vec.x, is_boundary && boundary_type == 4u);
    let s_neigh_U_y = select(bc_neighbor_scalar(state[neigh_idx * 11u + 1u], state[owner * 11u + 1u], bc_kind[idx * 3u + 1u], bc_value[idx * 3u + 1u], d_own, is_boundary), state[owner * 11u + 1u] - (state[owner * 11u + 0u] * normal_vec.x + state[owner * 11u + 1u] * normal_vec.y) * normal_vec.y, is_boundary && boundary_type == 4u);
    let s_neigh_d_p = select(state[neigh_idx * 11u + 3u], state[owner * 11u + 3u], is_boundary);
    let s_neigh_grad_p_x = select(state[neigh_idx * 11u + 4u], state[owner * 11u + 4u], is_boundary);
    let s_neigh_grad_p_y = select(state[neigh_idx * 11u + 5u], state[owner * 11u + 5u], is_boundary);
    let s_neigh_p = bc_neighbor_scalar(state[neigh_idx * 11u + 2u], state[owner * 11u + 2u], bc_kind[idx * 3u + 2u], bc_value[idx * 3u + 2u], d_own, is_boundary);
    let s_neigh_rho = select(state[neigh_idx * 11u + 10u], state[owner * 11u + 10u], is_boundary);
