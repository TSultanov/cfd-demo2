...
### Incompressible backwards step (`simpleFoam`)
...
### Incompressible lid-driven cavity (`simpleFoam`)
...
- Thermo: ideal gas with `gamma≈1.4`, `mu=0` (Euler-like), `T=1`.
- BCs: inlet `U=(1 0 0)`, `p=1`, `T=1`; outlet `zeroGradient`; walls `U=(0 0 0)`.
...
- BCs: inlet `U=(2 0 0)`, `p=1`, `T=1`; outlet/top `zeroGradient`; ramp `U=(0 0 0)`.
