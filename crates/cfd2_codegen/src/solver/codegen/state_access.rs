...
// ============================================================================
// Name-Based State Access Helpers (delegate to slot-based)
// ============================================================================
...
fn compute_offset(slots: &ResolvedStateSlotsSpec, field: &str, component: u32) -> u32 {
    ...
    compute_offset_for_slot(slot, component)
}
...
pub fn state_component(
    ...
    state_component_slot(slots.stride, buffer, idx, slot, component)
}
...
pub fn state_vec2(
    ...
    state_vec2_slot(slots.stride, buffer, idx, slot)
}
...
pub fn state_vec3(
    ...
    state_vec3_slot(slots.stride, buffer, idx, slot)
}
...
pub fn state_component_typed(
    ...
    state_component_slot_typed(slots.stride, buffer, idx, slot, component)
}
...
pub fn state_scalar_typed(
    ...
    state_scalar_slot_typed(slots.stride, buffer, idx, slot)
}
...
pub fn state_vec2_typed(
    ...
    state_vec2_slot_typed(slots.stride, buffer, idx, slot)
}
...
pub fn state_vec3_typed(
    ...
    state_vec3_slot_typed(slots.stride, buffer, idx, slot)
}
...
pub fn state_scalar_dim<D: UnitDimension>(
    ...
    state_scalar_slot_dim(slots.stride, buffer, idx, slot)
}
...
pub fn state_vec2_dim<D: UnitDimension>(
    ...
    state_vec2_slot_dim(slots.stride, buffer, idx, slot)
}
...
pub fn state_vec3_dim<D: UnitDimension>(
    ...
    state_vec3_slot_dim(slots.stride, buffer, idx, slot)
}
...
pub fn state_component_dim<D: UnitDimension>(
    ...
    state_component_slot_dim(slots.stride, buffer, idx, slot, component)
}
...
mod tests {
    ...
    #[test]
    fn slot_based_state_access_works() {
        let slots = test_slots_from_fields(vec![
            ("U", PortFieldKind::Vector2, si::VELOCITY),
            ("p", PortFieldKind::Scalar, si::PRESSURE),
        ]);
    ...
    fn slot_based_typed_state_access_tracks_units() {
        ...
        // Test slot-based typed scalar access
        let p = state_scalar_slot_typed(slots.stride, "state", "idx", p_slot);
        assert_eq!(p.ty, DslType::f32());
        assert_eq!(p.unit, si::PRESSURE);
        assert_eq!(p.expr.to_string(), "state[idx * 3u + 2u]");
        
        // Test slot-based typed vec2 access
    ...
    #[test]
    fn slot_based_typed_dim_state_access_works() {
        let slots = test_slots_from_fields(vec![
            ("p", PortFieldKind::Scalar, si::PRESSURE),
            ("U", PortFieldKind::Vector2, si::VELOCITY),
        ]);
        ...
        // Test slot-based typed dimension scalar access
        let p: TypedExpr<Pressure> = state_scalar_slot_dim(slots.stride, "state", "idx", p_slot);
        assert_eq!(p.expr.to_string(), "state[idx * 3u + 0u]");
        
        // Test slot-based typed dimension vec2 access
        let u: TypedExpr<Velocity> = state_vec2_slot_dim(slots.stride, "state", "idx", u_slot);
        assert_eq!(
            u.expr.to_string(),
            "vec2<f32>(state[idx * 3u + 1u], state[idx * 3u + 2u])"
        );
    }
    ...
    #[test]
    fn name_based_delegates_to_slot_based() {
        // Verify that name-based helpers delegate correctly by checking they produce
        // the same results as direct slot-based access
        let slots = test_slots_from_fields(vec![
            ("U", PortFieldKind::Vector2, si::VELOCITY),
            ("p", PortFieldKind::Scalar, si::PRESSURE),
        ]);
        ...
        // Name-based
        let name_based = state_scalar_typed(&slots, "state", "idx", "p");
        
        // Slot-based
        let slot_based = state_scalar_slot_typed(slots.stride, "state", "idx", p_slot);
        
        assert_eq!(name_based.expr.to_string(), slot_based.expr.to_string());
        assert_eq!(name_based.unit, slot_based.unit);
        assert_eq!(name_based.ty, slot_based.ty);
    }
}
