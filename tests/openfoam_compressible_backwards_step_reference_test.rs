...
/// Test compressible backwards step against OpenFOAM reference.
///
...
/// (cfd2: 200 steps at the reference dt = endTime 4e-4 s; ~4% of one flow-through).
...
/// Run with: `cargo test --test openfoam_compressible_backwards_step_reference_test -- --ignored --timeout 120`
#[test]
#[ignore]
fn openfoam_compressible_backwards_step_matches_reference_field() {
    ...
    assert!(
        u_max.rel < common::reference_bands("compressible_backstep").max_cell_u,
        "U mismatch vs OpenFOAM (per-cell): max_rel={:.6} (tol={:.6}) max_abs={:.6} at (x={:.6}, y={:.6})",
        u_max.rel,
        common::reference_bands("compressible_backstep").max_cell_u,
        u_max.abs,
        sol_rows[u_max.idx].0,
        sol_rows[u_max.idx].1
    );
    assert!(
        p_max.rel < common::reference_bands("compressible_backstep").max_cell_p,
        "p mismatch vs OpenFOAM (per-cell): max_rel={:.6} (tol={:.6}) max_abs={:.3} at (x={:.6}, y={:.6})",
        p_max.rel,
        common::reference_bands("compressible_backstep").max_cell_p,
        p_max.abs,
