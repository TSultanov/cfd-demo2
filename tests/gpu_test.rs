...
fn test_gpu_solver_init() {
    ...
    pollster::block_on(async {
        ...
        let init_p: Vec<f64> = (0..mesh.num_cells()).map(|i| {
        ...
        for k in start..end {
            ...
            println!("  Face {}: Owner={}, Neighbor={:?}, Normal=({:.2}, {:.2}), Area={:.2}", 
        ...
        for (i, (gx, gy)) in result_grad_p.iter().enumerate() {
            let cx = mesh.cell_cx[i];
            let cy = mesh.cell_cy[i];
            println!("Cell {}: pos=({:.2}, {:.2}), grad=({:.2}, {:.2})", i, cx, cy, gx, gy);
            // Check if internal
        ...
        assert!(max_err < 0.1, "Gradient calculation error too high: {}", max_err);
        ...
        for &val in &matrix_values {
            if val > 1e-6 { count_pos += 1; }
            if val < -1e-6 { count_neg += 1; }
        }
        
        assert!(count_pos >= 64, "Expected at least 64 positive diagonal entries");
        assert!(count_neg >= 200, "Expected many negative off-diagonal entries");
        
        // Test Linear Solver
        // Set U = (x, y) so div(U) = 2
        let u_div: Vec<(f64, f64)> = (0..mesh.num_cells()).map(|i| {
        ...
        assert!(max_x > 0.0, "Solver should produce non-zero correction for non-zero divergence");
    });
}
