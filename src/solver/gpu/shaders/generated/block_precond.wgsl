...
fn swap_rows(a: ptr<function, array<array<f32, MAX_BLOCK>, MAX_BLOCK>>, b: ptr<function, array<array<f32, MAX_BLOCK>, MAX_BLOCK>>, r0: u32, r1: u32, n: u32) {
    if (r0 == r1) {
        return;
    }
...
fn build_block_inv(@builtin(global_invocation_id) global_id: vec3<u32>, @builtin(num_workgroups) num_workgroups: vec3<u32>) {
    ...
    for (var i = 0u; i < b; i = i + 1u) {
        ...
        if (pivot_val < 0.000000000001) {
            singular = true;
        }
        swap_rows(&a, &inv, i, pivot, b);
        var piv = a[i][i];
        if (abs(piv) < 0.000000000001) {
            piv = select(0.000000000001, -0.000000000001, piv < 0.0);
        }
        let inv_piv = 1.0 / piv;
        for (var c = 0u; c < b; c = c + 1u) {
            a[i][c] = a[i][c] * inv_piv;
...
@compute
@workgroup_size(64)
fn apply_block_precond(@builtin(global_invocation_id) global_id: vec3<u32>, @builtin(num_workgroups) num_workgroups: vec3<u32>) {
    let stride_x = num_workgroups.x * 64u;
    let cell = global_id.y * stride_x + global_id.x;
    if (cell >= params.num_cells) {
        return;
