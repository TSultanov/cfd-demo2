...
fn generate_wgsl_binding_meta(manifest_dir: &str) {
    ...
    for path in list_wgsl_files_in_dir(&shader_dir) {
        let stem = path
            .file_stem()
            .and_then(|s| s.to_str())
            .unwrap_or_else(|| panic!("bad WGSL file name: {}", path.display()));
        let src = fs::read_to_string(&path)
            .unwrap_or_else(|err| panic!("failed to read WGSL '{}': {err}", path.display()));
        let bindings = parse_wgsl_bindings(&src);

        let ident = sanitize_rust_ident(stem).to_ascii_uppercase();
...
fn generate_kernel_registry_map(
    ...
    let shader_dir = PathBuf::from(manifest_dir)
        ...
        .join("gpu")
        .join("shaders");
    let mut id_only_entries: Vec<(String, String, String)> = Vec::new();
    for path in list_wgsl_files_in_dir(&shader_dir) {
        let stem = path
            .file_stem()
            .and_then(|s| s.to_str())
...
fn list_wgsl_files_in_dir(dir: &PathBuf) -> Vec<PathBuf> {
