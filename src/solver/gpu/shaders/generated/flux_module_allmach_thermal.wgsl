...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    let s_own_U_x = select(bc_neighbor_scalar(state[owner * 15u + 0u], state[owner * 15u + 0u], bc_kind[idx * 4u + 0u], bc_value[idx * 4u + 0u], d_own, is_boundary), state[owner * 15u + 0u] - (state[owner * 15u + 0u] * normal_vec.x + state[owner * 15u + 1u] * normal_vec.y) * normal_vec.x, is_boundary && boundary_type == 4u);
    let s_own_U_y = select(bc_neighbor_scalar(state[owner * 15u + 1u], state[owner * 15u + 1u], bc_kind[idx * 4u + 1u], bc_value[idx * 4u + 1u], d_own, is_boundary), state[owner * 15u + 1u] - (state[owner * 15u + 0u] * normal_vec.x + state[owner * 15u + 1u] * normal_vec.y) * normal_vec.y, is_boundary && boundary_type == 4u);
    let s_own_d_p = state[owner * 15u + 3u];
    let s_own_grad_p_x = state[owner * 15u + 4u];
    let s_own_grad_p_y = state[owner * 15u + 5u];
    let s_own_p = bc_neighbor_scalar(state[owner * 15u + 2u], state[owner * 15u + 2u], bc_kind[idx * 4u + 2u], bc_value[idx * 4u + 2u], d_own, is_boundary);
    let s_own_rho = state[owner * 15u + 10u];
    let s_neigh_U_x = select(bc_neighbor_scalar(state[neigh_idx * 15u + 0u], state[owner * 15u + 0u], bc_kind[idx * 4u + 0u], bc_value[idx * 4u + 0u], d_own, is_boundary), state[owner * 15u + 0u] - (state[owner * 15u + 0u] * normal_vec.x + state[owner * 15u + 1u] * normal_vec.y) * normal_vec.x, is_boundary && boundary_type == 4u);
    let s_neigh_U_y = select(bc_neighbor_scalar(state[neigh_idx * 15u + 1u], state[owner * 15u + 1u], bc_kind[idx * 4u + 1u], bc_value[idx * 4u + 1u], d_own, is_boundary), state[owner * 15u + 1u] - (state[owner * 15u + 0u] * normal_vec.x + state[owner * 15u + 1u] * normal_vec.y) * normal_vec.y, is_boundary && boundary_type == 4u);
    ...
    let s_neigh_p = bc_neighbor_scalar(state[neigh_idx * 15u + 2u], state[owner * 15u + 2u], bc_kind[idx * 4u + 2u], bc_value[idx * 4u + 2u], d_own, is_boundary);
    let s_neigh_rho = select(state[neigh_idx * 15u + 10u], state[owner * 15u + 10u], is_boundary);
    fluxes[idx * 4u + 0u] = (s_own_rho * lambda + s_neigh_rho * lambda_other) * dot(vec2<f32>(s_own_U_x, s_own_U_y) * lambda + vec2<f32>(s_neigh_U_x, s_neigh_U_y) * lambda_other + (vec2<f32>(s_own_grad_p_x, s_own_grad_p_y) * lambda + vec2<f32>(select(s_neigh_grad_p_x, 0.0, is_boundary && !(boundary_type == 2u)), select(s_neigh_grad_p_y, 0.0, is_boundary && !(boundary_type == 2u))) * lambda_other) * (s_own_d_p * lambda + s_neigh_d_p * lambda_other), normal_vec) * area - (s_own_rho * lambda + s_neigh_rho * lambda_other) * (s_own_d_p * lambda + s_neigh_d_p * lambda_other) * (s_neigh_p - s_own_p) / dist * area;
