...
pub fn generate_prepare_coupled_wgsl(layout: &StateLayout) -> String {
...
fn main_body(layout: &StateLayout) -> Block {
    ...
    let u_old_expr = state_vec2_expr(layout, "state_old", "idx", FIELD_U);
    let u_idx_expr = state_vec2_expr(layout, "state", "idx", FIELD_U);
    let p_idx_expr = state_scalar_expr(layout, "state", "idx", FIELD_P);
    ...
    let u_neigh_expr = state_vec2_expr(layout, "state", "n_idx", FIELD_U);
    let dp_owner_expr = state_scalar_expr(layout, "state", "owner", FIELD_D_P);
    let dp_neigh_expr = state_scalar_expr(layout, "state", "n_idx", FIELD_D_P);
    let gp_owner_expr = state_vec2_expr(layout, "state", "owner", FIELD_GRAD_P);
    let gp_neigh_expr = state_vec2_expr(layout, "state", "n_idx", FIELD_GRAD_P);
    let p_owner_expr = state_scalar_expr(layout, "state", "owner", FIELD_P);
    let p_neigh_expr = state_scalar_expr(layout, "state", "n_idx", FIELD_P);
    ...
    let inlet_block = dsl::block(vec![
        ...
        dsl::assign(
            "flux",
            "constants.density * (u_bc.x * normal_flux.x + u_bc.y * normal_flux.y) * area",
        ),
    ]);
    ...
    let outlet_block = dsl::block(vec![
        ...
        dsl::let_("raw_flux", "constants.density * u_n * area"),
    ...
    loop_body.push(dsl::if_block(
        ...
        dsl::block(vec![
            ...
            dsl::assign("flux", "constants.density * (u_n * area + rc_term)"),
        ]),
    ...
    loop_body.push(dsl::let_("diff_coeff", "mu * area / dist"));
    ...
    let val_other_p_expr = state_scalar_expr(layout, "state", "other_idx", FIELD_P);
    ...
    let u_other_expr = state_vec2_expr(layout, "state", "other_idx", FIELD_U);
    ...
    let d_p_target = state_component_expr(layout, "state", "idx", FIELD_D_P, 0);
    stmts.push(dsl::if_block(
    ...
    let grad_p_x = state_component_expr(layout, "state", "idx", FIELD_GRAD_P, 0);
    let grad_p_y = state_component_expr(layout, "state", "idx", FIELD_GRAD_P, 1);
...
mod tests {
    ...
    fn prepare_coupled_codegen_emits_state_arrays() {
        ...
        let wgsl = generate_prepare_coupled_wgsl(&layout);
