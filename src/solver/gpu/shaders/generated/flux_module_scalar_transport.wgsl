...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    let s_neigh_U_adv_y = select(state[neigh_idx * 4u + 2u], state[owner * 4u + 2u], is_boundary);
    fluxes[idx * 1u + 0u] = dot(vec2<f32>(s_own_U_adv_x, s_own_U_adv_y) * lambda + vec2<f32>(s_neigh_U_adv_x, s_neigh_U_adv_y) * lambda_other, normal_vec) * area;
}
