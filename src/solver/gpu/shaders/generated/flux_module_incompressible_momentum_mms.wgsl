...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    let s_own_U_x = select(bc_neighbor_scalar(state[owner * 10u + 0u], state[owner * 10u + 0u], bc_kind[idx * 3u + 0u], bc_value[idx * 3u + 0u], d_own, is_boundary), state[owner * 10u + 0u] - (state[owner * 10u + 0u] * normal_vec.x + state[owner * 10u + 1u] * normal_vec.y) * normal_vec.x, is_boundary && boundary_type == 4u);
    let s_own_U_y = select(bc_neighbor_scalar(state[owner * 10u + 1u], state[owner * 10u + 1u], bc_kind[idx * 3u + 1u], bc_value[idx * 3u + 1u], d_own, is_boundary), state[owner * 10u + 1u] - (state[owner * 10u + 0u] * normal_vec.x + state[owner * 10u + 1u] * normal_vec.y) * normal_vec.y, is_boundary && boundary_type == 4u);
    let s_own_d_p = state[owner * 10u + 3u];
    let s_own_grad_p_x = state[owner * 10u + 4u];
    let s_own_grad_p_y = state[owner * 10u + 5u];
    let s_neigh_U_x = select(bc_neighbor_scalar(state[neigh_idx * 10u + 0u], state[owner * 10u + 0u], bc_kind[idx * 3u + 0u], bc_value[idx * 3u + 0u], d_own, is_boundary), state[owner * 10u + 0u] - (state[owner * 10u + 0u] * normal_vec.x + state[owner * 10u + 1u] * normal_vec.y) * normal_vec.x, is_boundary && boundary_type == 4u);
    let s_neigh_U_y = select(bc_neighbor_scalar(state[neigh_idx * 10u + 1u], state[owner * 10u + 1u], bc_kind[idx * 3u + 1u], bc_value[idx * 3u + 1u], d_own, is_boundary), state[owner * 10u + 1u] - (state[owner * 10u + 0u] * normal_vec.x + state[owner * 10u + 1u] * normal_vec.y) * normal_vec.y, is_boundary && boundary_type == 4u);
    let s_neigh_d_p = select(state[neigh_idx * 10u + 3u], state[owner * 10u + 3u], is_boundary);
    let s_neigh_grad_p_x = select(state[neigh_idx * 10u + 4u], state[owner * 10u + 4u], is_boundary);
    let s_neigh_grad_p_y = select(state[neigh_idx * 10u + 5u], state[owner * 10u + 5u], is_boundary);
    let s_neigh_p = bc_neighbor_scalar(state[neigh_idx * 10u + 2u], state[owner * 10u + 2u], bc_kind[idx * 3u + 2u], bc_value[idx * 3u + 2u], d_own, is_boundary);
    fluxes[idx * 3u + 0u] = constants.density * dot(vec2<f32>(s_own_U_x, s_own_U_y) * lambda + vec2<f32>(s_neigh_U_x, s_neigh_U_y) * lambda_other, normal_vec) * area + ((1.0 - select(0.0, 1.0, is_boundary)) * (dot((vec2<f32>(s_own_grad_p_x, s_own_grad_p_y) * s_own_d_p * lambda + vec2<f32>(select(s_neigh_grad_p_x, 0.0, false), select(s_neigh_grad_p_y, 0.0, false)) * s_neigh_d_p * lambda_other) * constants.density, c_neigh_cell_vec - c_owner_vec) / dist) + select(0.0, 1.0, is_boundary) * (select(0.0, 1.0, is_boundary && bc_kind[idx * 3u + 2u] == 1u) * (dot((vec2<f32>(s_own_grad_p_x, s_own_grad_p_y) * s_own_d_p * lambda + vec2<f32>(select(s_neigh_grad_p_x, 0.0, false), select(s_neigh_grad_p_y, 0.0, false)) * s_neigh_d_p * lambda_other) * constants.density, face_center_vec - c_owner_vec) / dist) + (1.0 - select(0.0, 1.0, is_boundary && bc_kind[idx * 3u + 2u] == 1u)) * (constants.density * (s_own_d_p * lambda + s_neigh_d_p * lambda_other) * ((s_neigh_p - state[owner * 10u + 2u]) / dist)))) * area - constants.density * (s_own_d_p * lambda + s_neigh_d_p * lambda_other) * ((s_neigh_p - state[owner * 10u + 2u]) / dist) * area;
