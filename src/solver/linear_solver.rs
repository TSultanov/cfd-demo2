...
impl<T: Float> SparseMatrix<T> {
    ...
    pub fn from_triplets(n_rows: usize, n_cols: usize, triplets: &[(usize, usize, T)]) -> Self {
        ...
        mat.values = vec![T::zero(); total_entries];
        mat.col_indices = vec![0; total_entries];

        // Fill in consolidated values, sorting columns within each row for consistency
        for r in 0..n_rows {
