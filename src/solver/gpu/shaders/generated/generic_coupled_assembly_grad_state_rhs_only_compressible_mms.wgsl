...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    rhs_0 += vol / constants.dt * state_old[idx * 26u + 0u];
    if (constants.time_scheme == 1u) {
        ...
        rhs_0 = rhs_0 - vol / constants.dt * state_old[idx * 26u + 0u] + vol / constants.dt * (factor_n * state_old[idx * 26u + 0u] - factor_nm1 * state_old_old[idx * 26u + 0u]);
    }
    if (constants.dtau > 0.0) {
        rhs_0 += dual_time_scale * state_iter[idx * 26u + 0u];
    }
    rhs_1 += vol / constants.dt * state_old[idx * 26u + 1u];
    if (constants.time_scheme == 1u) {
        let r = constants.dt / constants.dt_old;
        let diag_bdf2 = vol / constants.dt * (r * 2.0 + 1.0) / (r + 1.0);
        let factor_n = r + 1.0;
        let factor_nm1 = r * r / (r + 1.0);
        rhs_1 = rhs_1 - vol / constants.dt * state_old[idx * 26u + 1u] + vol / constants.dt * (factor_n * state_old[idx * 26u + 1u] - factor_nm1 * state_old_old[idx * 26u + 1u]);
    }
    if (constants.dtau > 0.0) {
        rhs_1 += dual_time_scale * state_iter[idx * 26u + 1u];
    }
    rhs_2 += vol / constants.dt * state_old[idx * 26u + 2u];
    if (constants.time_scheme == 1u) {
        let r = constants.dt / constants.dt_old;
        let diag_bdf2 = vol / constants.dt * (r * 2.0 + 1.0) / (r + 1.0);
        let factor_n = r + 1.0;
        let factor_nm1 = r * r / (r + 1.0);
        rhs_2 = rhs_2 - vol / constants.dt * state_old[idx * 26u + 2u] + vol / constants.dt * (factor_n * state_old[idx * 26u + 2u] - factor_nm1 * state_old_old[idx * 26u + 2u]);
    }
    if (constants.dtau > 0.0) {
        rhs_2 += dual_time_scale * state_iter[idx * 26u + 2u];
    }
    rhs_3 += vol / constants.dt * state_old[idx * 26u + 7u];
    if (constants.time_scheme == 1u) {
        let r = constants.dt / constants.dt_old;
        let diag_bdf2 = vol / constants.dt * (r * 2.0 + 1.0) / (r + 1.0);
        let factor_n = r + 1.0;
        let factor_nm1 = r * r / (r + 1.0);
        rhs_3 = rhs_3 - vol / constants.dt * state_old[idx * 26u + 7u] + vol / constants.dt * (factor_n * state_old[idx * 26u + 7u] - factor_nm1 * state_old_old[idx * 26u + 7u]);
    }
    if (constants.dtau > 0.0) {
        rhs_3 += dual_time_scale * state_iter[idx * 26u + 7u];
    }
    ...
    for (var k = start; k < end; k++) {
        ...
        if (!is_boundary) {
            ...
            if (boundary_type == 4u) {
                rhs_1 += diff_coeff_rho_u * (state[idx * 26u + 10u] - (state[idx * 26u + 10u] * normal.x + state[idx * 26u + 11u] * normal.y) * normal.x);
            } else {
        ...
        if (!is_boundary) {
            ...
            if (boundary_type == 4u) {
                rhs_2 += diff_coeff_rho_u * (state[idx * 26u + 11u] - (state[idx * 26u + 10u] * normal.x + state[idx * 26u + 11u] * normal.y) * normal.y);
            } else {
