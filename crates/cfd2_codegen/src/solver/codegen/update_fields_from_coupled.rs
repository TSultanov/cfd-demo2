...
use super::{coupled_incompressible_fields::derive_coupled_incompressible_fields, ir::DiscreteSystem};
...
pub fn generate_update_fields_from_coupled_wgsl(
    ...
    let fields = derive_coupled_incompressible_fields(system, layout)
        .unwrap_or_else(|e| panic!("update_fields_from_coupled: {e}"));
    ...
    module.push(Item::Function(main_fn(layout, &fields.u, &fields.p)));
    module.to_wgsl()
}
...
fn main_fn(layout: &StateLayout, momentum_field: &str, pressure_field: &str) -> Function {
    let params = vec![
        Param::new(
            "global_id",
            Type::vec3_u32(),
    ...
    Function::new(
        ...
        main_body(layout, momentum_field, pressure_field),
    )
}

fn main_body(layout: &StateLayout, momentum_field: &str, pressure_field: &str) -> Block {
    ...
    let p_components = layout
        .field(p_field)
        .unwrap_or_else(|| panic!("missing state field '{p_field}'"))
        .component_count() as usize;
    if u_components != 2 {
        panic!(
...
mod tests {
    ...
    use crate::solver::ir::{vol_scalar, vol_vector, Equation, EquationSystem, SchemeRegistry};
    ...
    fn update_fields_codegen_emits_state_arrays() {
        ...
        system.add_equation(Equation::new(u));
        system.add_equation(Equation::new(p));
        ...
        let wgsl = generate_update_fields_from_coupled_wgsl(&discrete, &layout);
