...
fn test_matrix_diagonals() {
    ...
    println!("Mesh has {} cells, {} faces", mesh.num_cells(), mesh.num_faces());
    ...
    let (mat, rhs) = Fvm::assemble_scalar_transport(
        &mesh, &phi, &fluxes, gamma, dt, &Scheme::Upwind, bc, None, None, None,
    );
    ...
    for row in 0..mat.n_rows {
        ...
        let unique_count = col_indices_in_row.iter().collect::<std::collections::HashSet<_>>().len();
        if unique_count != col_indices_in_row.len() {
            ...
            for (col, count) in counts {
                if count > 1 {
                    ...
                    let vals: Vec<f64> = entries.iter().filter(|(c, _)| *c == col).map(|(_, v)| *v).collect();
                    println!("    Values: {:?}", vals);
...
fn test_diagonal_sum_vs_expected() {
    ...
    for f in 0..mesh.num_faces() {
        println!("Face {}: owner={}, neighbor={:?}, boundary={:?}, area={:.3}", 
                 f, mesh.face_owner[f], mesh.face_neighbor[f], 
                 mesh.face_boundary[f], mesh.face_area[f]);
    }
    ...
    for row in 0..mat.n_rows {
        ...
        for k in start..end {
            if mat.col_indices[k] == row {
                ...
                println!("  Row {}: diagonal entry #{} = {:.6}", row, diag_count, mat.values[k]);
            }
        }
        ...
        println!("  Row {}: expected V/dt = {:.6}, actual diag_sum = {:.6}, count = {}", 
...
fn test_triplet_duplicates() {
    ...
    assert_eq!(mat.row_offsets[1] - mat.row_offsets[0], 2, "Row 0 should have exactly 2 entries after consolidation");
    
    // Row 1 should have 2 entries: (1,0)=-0.5 and (1,1)=1.0
    assert_eq!(mat.row_offsets[2] - mat.row_offsets[1], 2, "Row 1 should have 2 entries");
    ...
    for row in 0..mat.n_rows {
        ...
        for k in start..end-1 {
            assert!(mat.col_indices[k] < mat.col_indices[k+1], 
                    "Column indices should be sorted within row {}", row);
        }
    }
