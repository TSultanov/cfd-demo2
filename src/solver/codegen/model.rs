...
mod tests {
    ...
    fn incompressible_momentum_system_contains_expected_terms() {
        ...
        let eqn = &system.equations()[0];
        assert_eq!(eqn.target().name(), "U");
        assert_eq!(eqn.terms().len(), 4);
        assert_eq!(eqn.terms()[0].op, TermOp::Ddt);
        assert_eq!(eqn.terms()[1].op, TermOp::Div);
        assert_eq!(eqn.terms()[2].op, TermOp::Laplacian);
        assert_eq!(eqn.terms()[3].op, TermOp::Grad);
    }
    ...
    fn incompressible_momentum_model_includes_state_layout() {
        ...
        assert_eq!(model.system.equations().len(), 1);
    }
}
