...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    for (var face_offset = start; face_offset < end; face_offset++) {
        ...
        if (!is_boundary) {
            ...
            matrix_values[start_row_0 + 4u * neighbor_rank + 0u] = a_prod_scaled * area;
            matrix_values[start_row_0 + 4u * neighbor_rank + 1u] = normal.x * a_r * area;
            matrix_values[start_row_0 + 4u * neighbor_rank + 2u] = normal.y * a_r * area;
            matrix_values[start_row_0 + 4u * neighbor_rank + 3u] = 0.0 * area;
            matrix_values[start_row_1 + 4u * neighbor_rank + 0u] = ((dp_drho_r * normal.x - u_r_x * u_n_r) * a_r + d_diff_x_r_rho) * area;
            matrix_values[start_row_1 + 4u * neighbor_rank + 1u] = ((u_n_r + normal.x * (u_r_x + dp_dru_r)) * a_r + d_diff_x_r_ru + a_prod_scaled) * area;
            matrix_values[start_row_1 + 4u * neighbor_rank + 2u] = ((u_r_x * normal.y + dp_drv_r * normal.x) * a_r + d_diff_x_r_rv) * area;
            matrix_values[start_row_1 + 4u * neighbor_rank + 3u] = (dp_dre_r * normal.x * a_r + d_diff_x_r_re) * area;
            matrix_values[start_row_2 + 4u * neighbor_rank + 0u] = ((dp_drho_r * normal.y - u_r_y * u_n_r) * a_r + d_diff_y_r_rho) * area;
            matrix_values[start_row_2 + 4u * neighbor_rank + 1u] = ((u_r_y * normal.x + dp_dru_r * normal.y) * a_r + d_diff_y_r_ru) * area;
            matrix_values[start_row_2 + 4u * neighbor_rank + 2u] = ((u_n_r + normal.y * (u_r_y + dp_drv_r)) * a_r + d_diff_y_r_rv + a_prod_scaled) * area;
            matrix_values[start_row_2 + 4u * neighbor_rank + 3u] = (dp_dre_r * normal.y * a_r + d_diff_y_r_re) * area;
            matrix_values[start_row_3 + 4u * neighbor_rank + 0u] = (u_n_r * (dp_drho_r - H_r) * a_r + d_e_visc_r_rho) * area;
            matrix_values[start_row_3 + 4u * neighbor_rank + 1u] = ((H_r * normal.x + dp_dru_r * u_n_r) * a_r + d_e_visc_r_ru) * area;
            matrix_values[start_row_3 + 4u * neighbor_rank + 2u] = ((H_r * normal.y + dp_drv_r * u_n_r) * a_r + d_e_visc_r_rv) * area;
            matrix_values[start_row_3 + 4u * neighbor_rank + 3u] = (1.4 * u_n_r * a_r + d_e_visc_r_re + a_prod_scaled) * area;
            diag_00 += (0.0 - a_prod_scaled) * area;
            diag_01 += normal.x * a_l * area;
            diag_02 += normal.y * a_l * area;
            diag_03 += 0.0 * area;
            diag_10 += ((dp_drho_l * normal.x - u_l_x * u_n_l) * a_l + d_diff_x_l_rho) * area;
            diag_11 += ((u_n_l + normal.x * (u_l_x + dp_dru_l)) * a_l + d_diff_x_l_ru - a_prod_scaled) * area;
            diag_12 += ((u_l_x * normal.y + dp_drv_l * normal.x) * a_l + d_diff_x_l_rv) * area;
            diag_13 += (dp_dre_l * normal.x * a_l + d_diff_x_l_re) * area;
            diag_20 += ((dp_drho_l * normal.y - u_l_y * u_n_l) * a_l + d_diff_y_l_rho) * area;
            diag_21 += ((u_l_y * normal.x + dp_dru_l * normal.y) * a_l + d_diff_y_l_ru) * area;
            diag_22 += ((u_n_l + normal.y * (u_l_y + dp_drv_l)) * a_l + d_diff_y_l_rv - a_prod_scaled) * area;
            diag_23 += (dp_dre_l * normal.y * a_l + d_diff_y_l_re) * area;
            diag_30 += (u_n_l * (dp_drho_l - H_l) * a_l + d_e_visc_l_rho) * area;
            diag_31 += ((H_l * normal.x + dp_dru_l * u_n_l) * a_l + d_e_visc_l_ru) * area;
            diag_32 += ((H_l * normal.y + dp_drv_l * u_n_l) * a_l + d_e_visc_l_rv) * area;
            diag_33 += (1.4 * u_n_l * a_l + d_e_visc_l_re - a_prod_scaled) * area;
        } else {
            if (boundary_type == 1u) {
                ...
                diag_02 += normal.y * a_l * area;
                diag_03 += 0.0 * area;
                diag_10 += ((dp_drho_l * normal.x - u_l_x * u_n_l) * a_l + d_diff_x_l_rho + (dp_drho_r * normal.x - u_r_x * u_n_r) * a_r + d_diff_x_r_rho + ((u_n_r + normal.x * (u_r_x + dp_dru_r)) * a_r + d_diff_x_r_ru + a_prod_scaled) * constants.inlet_velocity) * area;
                diag_11 += ((u_n_l + normal.x * (u_l_x + dp_dru_l)) * a_l + d_diff_x_l_ru - a_prod_scaled) * area;
                diag_12 += ((u_l_x * normal.y + dp_drv_l * normal.x) * a_l + d_diff_x_l_rv) * area;
                diag_13 += (dp_dre_l * normal.x * a_l + d_diff_x_l_re + dp_dre_r * normal.x * a_r + d_diff_x_r_re) * area;
                diag_20 += ((dp_drho_l * normal.y - u_l_y * u_n_l) * a_l + d_diff_y_l_rho + (dp_drho_r * normal.y - u_r_y * u_n_r) * a_r + d_diff_y_r_rho + ((u_r_y * normal.x + dp_dru_r * normal.y) * a_r + d_diff_y_r_ru) * constants.inlet_velocity) * area;
                diag_21 += ((u_l_y * normal.x + dp_dru_l * normal.y) * a_l + d_diff_y_l_ru) * area;
                diag_22 += ((u_n_l + normal.y * (u_l_y + dp_drv_l)) * a_l + d_diff_y_l_rv - a_prod_scaled) * area;
                diag_23 += (dp_dre_l * normal.y * a_l + d_diff_y_l_re + dp_dre_r * normal.y * a_r + d_diff_y_r_re) * area;
                diag_30 += (u_n_l * (dp_drho_l - H_l) * a_l + d_e_visc_l_rho + u_n_r * (dp_drho_r - H_r) * a_r + d_e_visc_r_rho + ((H_r * normal.x + dp_dru_r * u_n_r) * a_r + d_e_visc_r_ru) * constants.inlet_velocity) * area;
                diag_31 += ((H_l * normal.x + dp_dru_l * u_n_l) * a_l + d_e_visc_l_ru) * area;
                diag_32 += ((H_l * normal.y + dp_drv_l * u_n_l) * a_l + d_e_visc_l_rv) * area;
                diag_33 += (1.4 * u_n_l * a_l + d_e_visc_l_re - a_prod_scaled + 1.4 * u_n_r * a_r + d_e_visc_r_re + a_prod_scaled) * area;
            } else {
                if (boundary_type == 3u) {
                    ...
                    diag_03 += 0.0 * area;
                    diag_10 += ((dp_drho_l * normal.x - u_l_x * u_n_l) * a_l + d_diff_x_l_rho + (dp_drho_r * normal.x - u_r_x * u_n_r) * a_r + d_diff_x_r_rho) * area;
                    diag_11 += ((u_n_l + normal.x * (u_l_x + dp_dru_l)) * a_l + d_diff_x_l_ru - a_prod_scaled + ((u_n_r + normal.x * (u_r_x + dp_dru_r)) * a_r + d_diff_x_r_ru + a_prod_scaled) * r11 + ((u_r_x * normal.y + dp_drv_r * normal.x) * a_r + d_diff_x_r_rv) * r21) * area;
                    diag_12 += ((u_l_x * normal.y + dp_drv_l * normal.x) * a_l + d_diff_x_l_rv + ((u_n_r + normal.x * (u_r_x + dp_dru_r)) * a_r + d_diff_x_r_ru + a_prod_scaled) * r12 + ((u_r_x * normal.y + dp_drv_r * normal.x) * a_r + d_diff_x_r_rv) * r22) * area;
                    diag_13 += (dp_dre_l * normal.x * a_l + d_diff_x_l_re + dp_dre_r * normal.x * a_r + d_diff_x_r_re) * area;
                    diag_20 += ((dp_drho_l * normal.y - u_l_y * u_n_l) * a_l + d_diff_y_l_rho + (dp_drho_r * normal.y - u_r_y * u_n_r) * a_r + d_diff_y_r_rho) * area;
                    diag_21 += ((u_l_y * normal.x + dp_dru_l * normal.y) * a_l + d_diff_y_l_ru + ((u_r_y * normal.x + dp_dru_r * normal.y) * a_r + d_diff_y_r_ru) * r11 + ((u_n_r + normal.y * (u_r_y + dp_drv_r)) * a_r + d_diff_y_r_rv + a_prod_scaled) * r21) * area;
                    diag_22 += ((u_n_l + normal.y * (u_l_y + dp_drv_l)) * a_l + d_diff_y_l_rv - a_prod_scaled + ((u_r_y * normal.x + dp_dru_r * normal.y) * a_r + d_diff_y_r_ru) * r12 + ((u_n_r + normal.y * (u_r_y + dp_drv_r)) * a_r + d_diff_y_r_rv + a_prod_scaled) * r22) * area;
                    diag_23 += (dp_dre_l * normal.y * a_l + d_diff_y_l_re + dp_dre_r * normal.y * a_r + d_diff_y_r_re) * area;
                    diag_30 += (u_n_l * (dp_drho_l - H_l) * a_l + d_e_visc_l_rho + u_n_r * (dp_drho_r - H_r) * a_r + d_e_visc_r_rho) * area;
                    diag_31 += ((H_l * normal.x + dp_dru_l * u_n_l) * a_l + d_e_visc_l_ru + ((H_r * normal.x + dp_dru_r * u_n_r) * a_r + d_e_visc_r_ru) * r11 + ((H_r * normal.y + dp_drv_r * u_n_r) * a_r + d_e_visc_r_rv) * r21) * area;
                    diag_32 += ((H_l * normal.y + dp_drv_l * u_n_l) * a_l + d_e_visc_l_rv + ((H_r * normal.x + dp_dru_r * u_n_r) * a_r + d_e_visc_r_ru) * r12 + ((H_r * normal.y + dp_drv_r * u_n_r) * a_r + d_e_visc_r_rv) * r22) * area;
                    diag_33 += (1.4 * u_n_l * a_l + d_e_visc_l_re - a_prod_scaled + 1.4 * u_n_r * a_r + d_e_visc_r_re + a_prod_scaled) * area;
                } else {
                    diag_00 += (0.0 - a_prod_scaled + a_prod_scaled) * area;
                    ...
                    diag_03 += 0.0 * area;
                    diag_10 += ((dp_drho_l * normal.x - u_l_x * u_n_l) * a_l + d_diff_x_l_rho + (dp_drho_r * normal.x - u_r_x * u_n_r) * a_r + d_diff_x_r_rho) * area;
                    diag_11 += ((u_n_l + normal.x * (u_l_x + dp_dru_l)) * a_l + d_diff_x_l_ru - a_prod_scaled + (u_n_r + normal.x * (u_r_x + dp_dru_r)) * a_r + d_diff_x_r_ru + a_prod_scaled) * area;
                    diag_12 += ((u_l_x * normal.y + dp_drv_l * normal.x) * a_l + d_diff_x_l_rv + (u_r_x * normal.y + dp_drv_r * normal.x) * a_r + d_diff_x_r_rv) * area;
                    diag_13 += (dp_dre_l * normal.x * a_l + d_diff_x_l_re + dp_dre_r * normal.x * a_r + d_diff_x_r_re) * area;
                    diag_20 += ((dp_drho_l * normal.y - u_l_y * u_n_l) * a_l + d_diff_y_l_rho + (dp_drho_r * normal.y - u_r_y * u_n_r) * a_r + d_diff_y_r_rho) * area;
                    diag_21 += ((u_l_y * normal.x + dp_dru_l * normal.y) * a_l + d_diff_y_l_ru + (u_r_y * normal.x + dp_dru_r * normal.y) * a_r + d_diff_y_r_ru) * area;
                    diag_22 += ((u_n_l + normal.y * (u_l_y + dp_drv_l)) * a_l + d_diff_y_l_rv - a_prod_scaled + (u_n_r + normal.y * (u_r_y + dp_drv_r)) * a_r + d_diff_y_r_rv + a_prod_scaled) * area;
                    diag_23 += (dp_dre_l * normal.y * a_l + d_diff_y_l_re + dp_dre_r * normal.y * a_r + d_diff_y_r_re) * area;
                    diag_30 += (u_n_l * (dp_drho_l - H_l) * a_l + d_e_visc_l_rho + u_n_r * (dp_drho_r - H_r) * a_r + d_e_visc_r_rho) * area;
                    diag_31 += ((H_l * normal.x + dp_dru_l * u_n_l) * a_l + d_e_visc_l_ru + (H_r * normal.x + dp_dru_r * u_n_r) * a_r + d_e_visc_r_ru) * area;
                    diag_32 += ((H_l * normal.y + dp_drv_l * u_n_l) * a_l + d_e_visc_l_rv + (H_r * normal.y + dp_drv_r * u_n_r) * a_r + d_e_visc_r_rv) * area;
                    diag_33 += (1.4 * u_n_l * a_l + d_e_visc_l_re - a_prod_scaled + 1.4 * u_n_r * a_r + d_e_visc_r_re + a_prod_scaled) * area;
                }
            }
        }
    ...
    matrix_values[start_row_0 + 4u * diag_rank + 0u] = diag_00;
    matrix_values[start_row_0 + 4u * diag_rank + 1u] = diag_01;
    matrix_values[start_row_0 + 4u * diag_rank + 2u] = diag_02;
    matrix_values[start_row_0 + 4u * diag_rank + 3u] = diag_03;
    matrix_values[start_row_1 + 4u * diag_rank + 0u] = diag_10;
    matrix_values[start_row_1 + 4u * diag_rank + 1u] = diag_11;
    matrix_values[start_row_1 + 4u * diag_rank + 2u] = diag_12;
    matrix_values[start_row_1 + 4u * diag_rank + 3u] = diag_13;
    matrix_values[start_row_2 + 4u * diag_rank + 0u] = diag_20;
    matrix_values[start_row_2 + 4u * diag_rank + 1u] = diag_21;
    matrix_values[start_row_2 + 4u * diag_rank + 2u] = diag_22;
    matrix_values[start_row_2 + 4u * diag_rank + 3u] = diag_23;
    matrix_values[start_row_3 + 4u * diag_rank + 0u] = diag_30;
    matrix_values[start_row_3 + 4u * diag_rank + 1u] = diag_31;
    matrix_values[start_row_3 + 4u * diag_rank + 2u] = diag_32;
    matrix_values[start_row_3 + 4u * diag_rank + 3u] = diag_33;
