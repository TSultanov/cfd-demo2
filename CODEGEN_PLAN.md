...
- Replaced `GpuUnifiedSolver`’s internal backend branching enum with a boxed `GpuPlanInstance` trait object implemented by each plan (now defined in `src/solver/gpu/plans/plan_instance.rs`), reducing per-family orchestration code and preparing for a single model-driven program path (`src/solver/gpu/unified_solver.rs`).
...
- Centralized GPU readback staging-buffer reuse in `StagingBufferCache` and used it for both incompressible and compressible readback paths (reducing per-plan duplication and avoiding per-read allocations) (`src/solver/gpu/readback.rs`, `src/solver/gpu/plans/incompressible.rs`, `src/solver/gpu/plans/compressible.rs`, `src/solver/gpu/structs.rs`).
...
- Moved common FGMRES vector ops (`gpu_norm`) into `FgmresWorkspace` and removed duplicated norm implementations from compressible/coupled FGMRES plan code (`src/solver/gpu/linear_solver/fgmres.rs`, `src/solver/gpu/plans/compressible_fgmres.rs`, `src/solver/gpu/plans/coupled_fgmres.rs`).
- Introduced `KrylovSolveModule<P>` to bundle `FgmresWorkspace` with a pluggable `FgmresPreconditionerModule` and migrated coupled/compressible FGMRES resources to use it (`src/solver/gpu/modules/krylov_solve.rs`, `src/solver/gpu/plans/compressible_fgmres.rs`, `src/solver/gpu/plans/coupled_fgmres.rs`).
- Added `LinearSystemView` (ports + `PortSpace`) and migrated the generic Krylov solve entrypoint to accept it, so caller code no longer passes raw `wgpu::Buffer` for `x` (`src/solver/gpu/modules/linear_system.rs`, `src/solver/gpu/modules/krylov_solve.rs`).
- Added `KrylovSolveModule::rhs_norm()` and migrated coupled/compressible FGMRES setup to build `LinearSystemView` once and reuse it for norms and matrix wiring (`src/solver/gpu/modules/krylov_solve.rs`, `src/solver/gpu/plans/compressible_fgmres.rs`, `src/solver/gpu/plans/coupled_fgmres.rs`).
- Added `FgmresWorkspace::new_from_system(LinearSystemView)` so workspace creation no longer requires threading individual CSR buffer references (`src/solver/gpu/linear_solver/fgmres.rs`, `src/solver/gpu/plans/compressible_fgmres.rs`, `src/solver/gpu/plans/coupled_fgmres.rs`).
- Migrated `CoupledSchurModule::new` to accept a `LinearSystemView` (ports + port-space) instead of raw CSR buffers, further reducing cross-module buffer coupling (`src/solver/gpu/modules/coupled_schur.rs`, `src/solver/gpu/plans/coupled_fgmres.rs`).
- Added a typed buffer “port” system for lowered GPU resources (`PortSpace`) and migrated compressible linear-system buffers to it (block CSR + RHS/X as `LinearSystemPorts`, plus scalar CSR row offsets for assembly) (`src/solver/gpu/modules/ports.rs`, `src/solver/gpu/modules/linear_system.rs`, `src/solver/gpu/modules/compressible_lowering.rs`).
- Migrated compressible block-CSR buffers (row/col/value) into `PortSpace` and refactored `CompressibleKernelsModule`/compressible FGMRES to consume them via `LinearSystemPorts`/`LinearSystemView` (plus a scalar CSR row-offsets port) (`src/solver/gpu/modules/compressible_kernels.rs`, `src/solver/gpu/plans/compressible.rs`, `src/solver/gpu/plans/compressible_fgmres.rs`).
...
- Removed duplicate scalar CSR/values/RHS/X buffer fields from `GpuSolver` and routed scalar linear-system IO through `linear_ports`/`linear_port_space` instead (reducing legacy state and hard-coded buffer threading) (`src/solver/gpu/structs.rs`, `src/solver/gpu/init/mod.rs`, `src/solver/gpu/linear_solver/common.rs`, `src/solver/gpu/plans/incompressible.rs`).
- Updated coupled AMG setup to read the pressure CSR via typed ports/port-space (removing direct dependence on `GpuSolver`’s raw CSR buffers in the FGMRES plan code) (`src/solver/gpu/plans/coupled_fgmres.rs`).
...
- Encapsulated generic-coupled kernels (assembly/update) into `GenericCoupledKernelsModule` and migrated `GpuGenericCoupledSolver::step()` to use `ModuleGraph` dispatch; generic-coupled bind-group wiring now pulls linear buffers via `LinearSystemPorts` (`src/solver/gpu/modules/generic_coupled_kernels.rs`, `src/solver/gpu/plans/generic_coupled.rs`).
...
3. **Ports everywhere**
   - Status: compressible + coupled block-CSR + RHS/X are in `PortSpace` and are consumed via `LinearSystemPorts`/`LinearSystemView` (remaining coupled-only preconditioner/work buffers still live in `CoupledSolverResources`).
