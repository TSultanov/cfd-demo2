...
impl SolverDriver {
        ...
        let (cached_u, cached_p) = if compressible {
            ...
            if allmach {
                ...
                if thermal {
                    ...
                    // u_ref = k*U_inlet (the pseudo-acoustic velocity floor) and the 0/1
                    // enable mask (off when compressibility is off, so psi_precond==0).
                    let u_ref =
                        ALLMACH_PRECOND_MACH_K * params.inlet_velocity.abs() as f64;
    ...
    pub fn apply_params(&mut self, params: &RuntimeParams) {
        ...
        if self.allmach {
            ...
            // only; no-op where absent): u_ref tracks the inlet-velocity slider and the
            // 0/1 mask disables the acoustic preconditioner when compressibility is off.
            let _ = solver.set_field_scalar_current(
                "u_ref",
                &vec![ALLMACH_PRECOND_MACH_K * params.inlet_velocity.abs() as f64; n],
            );
            let _ = solver.set_field_scalar_current(
                "precond_mask",
    ...
    fn read_back(&mut self) -> Readback {
        ...
        let rho = if self.allmach {
            ...
            // Update, so a host write here (built from the constant reference psi) would
            // clobber the local sound-speed preconditioner. The non-thermal barotropic
            // model has no on-device recovery and still needs the host refresh.
            if !self.thermal {
                let psi_precond =
