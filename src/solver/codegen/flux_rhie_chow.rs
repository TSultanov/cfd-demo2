...
pub fn generate_flux_rhie_chow_wgsl(system: &DiscreteSystem, layout: &StateLayout) -> String {
...
fn main_body(layout: &StateLayout, plan: &MomentumPlan) -> Block {
    ...
    let u_owner_expr = state_vec2_expr(layout, "state", "owner", FIELD_U);
    let dp_owner_expr = state_scalar_expr(layout, "state", "owner", FIELD_D_P);
    let grad_p_owner_expr = state_vec2_expr(layout, "state", "owner", FIELD_GRAD_P);
    ...
    let interior_block = {
        let u_neigh_expr = state_vec2_expr(layout, "state", "neigh_idx", FIELD_U);
        let dp_neigh_expr = state_scalar_expr(layout, "state", "neigh_idx", FIELD_D_P);
        let grad_p_neigh_expr = state_vec2_expr(layout, "state", "neigh_idx", FIELD_GRAD_P);
        let p_owner_expr = state_scalar_expr(layout, "state", "owner", FIELD_P);
        let p_neigh_expr = state_scalar_expr(layout, "state", "neigh_idx", FIELD_P);
