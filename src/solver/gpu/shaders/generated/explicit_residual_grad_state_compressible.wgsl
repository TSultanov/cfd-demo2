...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    for (var k = start; k < end; k++) {
        ...
        if (!is_boundary) {
            rhs_1 += diff_coeff_exp_rho_u_u * (state[other_idx * 22u + 10u] - state[idx * 22u + 10u]);
        } else {
            if (bc_kind[face_idx * 8u + 4u] == 1u) {
                rhs_1 += diff_coeff_exp_rho_u_u * (bc_value[face_idx * 8u + 4u] - state[idx * 22u + 10u]);
            } else {
        ...
        if (!is_boundary) {
            rhs_2 += diff_coeff_exp_rho_u_u * (state[other_idx * 22u + 11u] - state[idx * 22u + 11u]);
        } else {
            if (bc_kind[face_idx * 8u + 5u] == 1u) {
                rhs_2 += diff_coeff_exp_rho_u_u * (bc_value[face_idx * 8u + 5u] - state[idx * 22u + 11u]);
            } else {
