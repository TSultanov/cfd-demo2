...
pub fn build_prolongation(aggregates: &[usize], num_aggregates: usize, fine_size: usize) -> CsrMatrix {
...
impl AmgResources {
    pub fn new(
        ...
        for level_idx in 0..max_levels {
            ...
            let b_matrix_row_offsets = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
                label: Some(&format!("AMG L{} Matrix Row Offsets", level_idx)),
                contents: bytemuck::cast_slice(&current_matrix.row_offsets),
                usage: wgpu::BufferUsages::STORAGE,
            });
            let b_matrix_col_indices = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            ...
            let bg_matrix = device.create_bind_group(&wgpu::BindGroupDescriptor {
                ...
                layout: &bgl_matrix,
                entries: &[
                    wgpu::BindGroupEntry { binding: 0, resource: b_matrix_row_offsets.as_entire_binding() },
                    wgpu::BindGroupEntry { binding: 1, resource: b_matrix_col_indices.as_entire_binding() },
                    wgpu::BindGroupEntry { binding: 2, resource: b_matrix_values.as_entire_binding() },
                ],
            });
            
            // Params buffer
            let params = AmgParams { n: n as u32, omega: 0.8, padding: [0; 2] };
            ...
            let bg_state = device.create_bind_group(&wgpu::BindGroupDescriptor {
                ...
                entries: &[
                    ...
                    wgpu::BindGroupEntry { binding: 1, resource: b_b.as_entire_binding() },
                    wgpu::BindGroupEntry { binding: 2, resource: b_r.as_entire_binding() },
                    wgpu::BindGroupEntry { binding: 3, resource: b_params.as_entire_binding() },
                ],
            });
            ...
            let create_csr_buffers = |mat: Option<&CsrMatrix>| {
                if let Some(m) = mat {
                    ...
                    // Dummy buffers
                    (
                        device.create_buffer(&wgpu::BufferDescriptor { label: None, size: 4, usage: wgpu::BufferUsages::STORAGE, mapped_at_creation: false }),
                        device.create_buffer(&wgpu::BufferDescriptor { label: None, size: 4, usage: wgpu::BufferUsages::STORAGE, mapped_at_creation: false }),
                        device.create_buffer(&wgpu::BufferDescriptor { label: None, size: 4, usage: wgpu::BufferUsages::STORAGE, mapped_at_creation: false }),
                    )
                }
            ...
            let bg_p = device.create_bind_group(&wgpu::BindGroupDescriptor {
                ...
                layout: &bgl_op,
                entries: &[
                    wgpu::BindGroupEntry { binding: 0, resource: b_p_row.as_entire_binding() },
                    wgpu::BindGroupEntry { binding: 1, resource: b_p_col.as_entire_binding() },
                    wgpu::BindGroupEntry { binding: 2, resource: b_p_val.as_entire_binding() },
                ],
            });
            ...
            let bg_r = device.create_bind_group(&wgpu::BindGroupDescriptor {
                ...
                layout: &bgl_op,
                entries: &[
                    wgpu::BindGroupEntry { binding: 0, resource: b_r_row.as_entire_binding() },
                    wgpu::BindGroupEntry { binding: 1, resource: b_r_col.as_entire_binding() },
                    wgpu::BindGroupEntry { binding: 2, resource: b_r_val.as_entire_binding() },
                ],
            });
    ...
    pub fn v_cycle(&self, encoder: &mut wgpu::CommandEncoder, device: &wgpu::Device) {
        ...
        for i in 0..num_levels - 1 {
            ...
            // 1. Pre-smooth
            {
                let mut pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor { label: Some("AMG Pre-smooth"), timestamp_writes: None });
            ...
            // 2. Compute Residual
            {
                let mut pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor { label: Some("AMG Residual"), timestamp_writes: None });
            ...
            let bg_cross = device.create_bind_group(&wgpu::BindGroupDescriptor {
                ...
                entries: &[
                    wgpu::BindGroupEntry { binding: 0, resource: coarse.b_b.as_entire_binding() },
                ],
            });
            
            {
                let mut pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor { label: Some("AMG Restrict"), timestamp_writes: None });
            ...
            {
                let mut pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor { label: Some("AMG Clear Coarse"), timestamp_writes: None });
        ...
        for _ in 0..10 {
             let mut pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor { label: Some("AMG Coarse Solve"), timestamp_writes: None });
             pass.set_pipeline(&self.pipeline_smooth);
        ...
        for i in (0..num_levels - 1).rev() {
            ...
            let bg_cross = device.create_bind_group(&wgpu::BindGroupDescriptor {
                ...
                layout: &self.bgl_cross,
                entries: &[
                    wgpu::BindGroupEntry { binding: 0, resource: coarse.b_x.as_entire_binding() },
                ],
            });
            
            {
                let mut pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor { label: Some("AMG Prolongate"), timestamp_writes: None });
            ...
            // Post-smooth
            {
                let mut pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor { label: Some("AMG Post-smooth"), timestamp_writes: None });
                pass.set_pipeline(&self.pipeline_smooth);
                pass.set_bind_group(0, &fine.bg_matrix, &[]);
                pass.set_bind_group(1, &fine.bg_state, &[]);
                pass.dispatch_workgroups(fine_groups, 1, 1);
