...
fn test_fine_mesh_divergence() {
    pollster::block_on(async {
        ...
        println!("Volume range: min={:.6e}, max={:.6e}, ratio={:.2}", min_vol, max_vol, vol_ratio);
        
        // Count very small cells
        let small_cell_count = mesh.cell_vol.iter().filter(|&&v| v < min_vol * 10.0).count();
        ...
        println!("Running comparison for {} steps with dt={}...", num_steps, dt);
        ...
        for step in 1..=num_steps {
            ...
            if step % compare_interval == 0 || step <= 5 {
                ...
                let gpu_max_u: f64 = u_gpu.iter().map(|(x, y)| x.abs().max(y.abs())).fold(0.0, f64::max);
                let gpu_has_nan = u_gpu.iter().any(|(x, y)| x.is_nan() || y.is_nan() || x.is_infinite() || y.is_infinite());
                let gpu_has_inf_p = p_gpu.iter().any(|&p| p.is_nan() || p.is_infinite());
                
                let cpu_max_u: f64 = cpu_solver.u.vx.iter().zip(cpu_solver.u.vy.iter())
                    .map(|(&x, &y)| (x as f64).abs().max((y as f64).abs())).fold(0.0, f64::max);
                let cpu_has_nan = cpu_solver.u.vx.iter().any(|x| x.is_nan() || x.is_infinite());
                
                if gpu_has_nan || gpu_has_inf_p || gpu_max_u > 1e6 {
                    println!("Step {}: GPU DIVERGED! max_u={:.6e}, has_nan={}, has_inf_p={}", 
                             step, gpu_max_u, gpu_has_nan, gpu_has_inf_p);
                    
                    // Find the problematic cell
                    for i in 0..n_cells {
                        if u_gpu[i].0.is_nan() || u_gpu[i].0.is_infinite() || u_gpu[i].0.abs() > 1e6 ||
                           u_gpu[i].1.is_nan() || u_gpu[i].1.is_infinite() || u_gpu[i].1.abs() > 1e6 {
                            println!("  Problematic cell {}: GPU u=({:.6e}, {:.6e}), CPU u=({:.6e}, {:.6e})",
                                     i, u_gpu[i].0, u_gpu[i].1, cpu_solver.u.vx[i], cpu_solver.u.vy[i]);
                            println!("    Cell vol={:.6e}, center=({:.6}, {:.6})", 
                ...
                println!("Step {}: Max Diff U={:.6e}, V={:.6e}, P={:.6e}", 
        ...
        // Fail if GPU diverged
        assert!(!gpu_diverged, "GPU solver diverged at step {} while CPU remained stable", divergence_step);
        
        assert!(max_diff_u_overall < tolerance, 
                "U velocity difference too large: {:.6e} > {:.6e}", max_diff_u_overall, tolerance);
        assert!(max_diff_v_overall < tolerance, 
                "V velocity difference too large: {:.6e} > {:.6e}", max_diff_v_overall, tolerance);
    });
}
...
fn test_gpu_divergence_debug() {
    pollster::block_on(async {
        ...
        let mut vol_idx: Vec<(f64, usize)> = mesh.cell_vol.iter().enumerate().map(|(i, &v)| (v, i)).collect();
        vol_idx.sort_by(|a, b| a.0.partial_cmp(&b.0).unwrap());
        println!("\nSmallest volume cells:");
        for (vol, idx) in vol_idx.iter().take(5) {
            println!("  Cell {}: vol={:.6e}, center=({:.6}, {:.6})", idx, vol, mesh.cell_cx[*idx], mesh.cell_cy[*idx]);
        }
        ...
        let p_init = gpu_solver.get_p().await;
        let fluxes_init = gpu_solver.get_fluxes().await;
        
        let u_max_init: f64 = u_init_check.iter().map(|(x, y)| x.abs().max(y.abs())).fold(0.0, f64::max);
        let u_has_nan_init = u_init_check.iter().any(|(x, y)| x.is_nan() || y.is_nan());
        let flux_has_nan_init = fluxes_init.iter().any(|f| f.is_nan());
        println!("Initial max |u|={:.6e}, has_nan_u={}, has_nan_flux={}", u_max_init, u_has_nan_init, flux_has_nan_init);
        ...
        if mat_has_nan || rhs_has_nan {
            ...
            for (vol, idx) in vol_idx.iter().take(5) {
                println!("    Cell {}: d_p={:.6e}, vol={:.6e}", idx, d_p_vals[*idx], vol);
            }
        }
        ...
        let u_max_after: f64 = u_after.iter().map(|(x, y)| x.abs().max(y.abs())).fold(0.0, f64::max);
        ...
        println!("\nAfter step 1:");
        println!("  max |u|={:.6e}, has_nan_u={}, has_nan_p={}, has_nan_flux={}", 
        ...
        if u_has_nan_after || p_has_nan_after {
            ...
            for i in 0..n_cells {
                if u_after[i].0.is_nan() || u_after[i].1.is_nan() || p_after[i].is_nan() {
                    if nan_cells < 10 {
                        println!("  Cell {} has NaN: u=({:.6e}, {:.6e}), p={:.6e}", 
        ...
        // Check flux range
        let flux_max: f32 = fluxes_after.iter().cloned().fold(f32::NEG_INFINITY, f32::max);
        ...
        for step in 2..=20 {
            ...
            let u_max: f64 = u_check.iter().map(|(x, y)| x.abs().max(y.abs())).fold(0.0, f64::max);
            ...
            println!("Step {}: max_u={:.6e}, nan_u={}, nan_p={}, nan_flux={}, p=[{:.2e}, {:.2e}]", 
            ...
            if has_nan_u || has_nan_p || has_nan_flux || u_max > 1e6 {
                ...
                for i in 0..n_cells.min(100) {
                    if u_check[i].0.is_nan() || u_check[i].1.is_nan() || p_check[i].is_nan() {
                        println!("  First NaN at cell {}: u=({:.6e}, {:.6e}), p={:.6e}", 
                ...
                for i in 0..n_cells {
                    if !u_check[i].0.is_nan() && (u_check[i].0.abs() > 100.0 || u_check[i].1.abs() > 100.0) {
                        if extreme_cells < 5 {
                            println!("  Extreme velocity at cell {}: u=({:.6e}, {:.6e})", 
        ...
        assert!(diverged_step == 0, "GPU solver diverged at step {}", diverged_step);
    });
}
...
fn test_cpu_vs_gpu_stability() {
    pollster::block_on(async {
        ...
        for step in 1..=10 {
            ...
            println!("Step {}: GPU max_u={:.6e}, nan={}, inf_p={}, p=[{:.2e}, {:.2e}]", 
        ...
        for step in 1..=10 {
            ...
            let cpu_max_u: f64 = cpu_solver_alone.u.vx.iter().zip(cpu_solver_alone.u.vy.iter())
                .map(|(&x, &y)| (x as f64).abs().max((y as f64).abs())).fold(0.0, f64::max);
            let cpu_has_nan = cpu_solver_alone.u.vx.iter().any(|x| x.is_nan() || x.is_infinite());
            
            let p_max: f64 = cpu_solver_alone.p.values.iter().map(|&v| v as f64).fold(f64::NEG_INFINITY, f64::max);
            let p_min: f64 = cpu_solver_alone.p.values.iter().map(|&v| v as f64).fold(f64::INFINITY, f64::min);
            
            println!("Step {}: CPU max_u={:.6e}, nan={}, p=[{:.2e}, {:.2e}]", 
...
fn test_stable_timestep() {
    ...
    async {
        ...
        println!("Mesh: {} cells, {} faces. Volume range: {:.6e} to {:.6e}", 
        ...
        println!("\nFine mesh dx=0.005 with GUI-like inlet width ({})!", inlet_width);
        println!("dt={}, CFL={:.2}, UPWIND scheme (GUI default)", dt, u_init_x * dt / dx);
        ...
        for step in 1..=50 {
            ...
            if step <= 5 || step % 10 == 0 || gpu_has_nan || gpu_max_u > 5.0 {
                println!("Step {}: GPU max_u={:.6e}, nan={}", step, gpu_max_u, gpu_has_nan);
            }
        ...
        for step in 1..=50 {
            ...
            if step <= 5 || step % 10 == 0 || cpu_has_nan || cpu_max_u > 5.0 {
                println!("Step {}: CPU max_u={:.6e}, nan={}", step, cpu_max_u, cpu_has_nan);
            }
