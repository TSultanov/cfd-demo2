...
@group(1) @binding(2) 
var<storage, read> dot_a0: array<f32>;

@group(1) @binding(3) 
var<storage, read> dot_b0: array<f32>;

@group(1) @binding(4) 
var<storage, read> dot_a1: array<f32>;

@group(1) @binding(5) 
var<storage, read> dot_b1: array<f32>;
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>, @builtin(local_invocation_id) local_id: vec3<u32>, @builtin(workgroup_id) group_id: vec3<u32>, @builtin(num_workgroups) num_workgroups: vec3<u32>) {
    ...
    if (lid == 0u) {
        ...
        if (group_flat < params.num_groups) {
            dot_result_a[group_flat] = scratch_a[0u];
            dot_result_b[group_flat] = scratch_b[0u];
        }
    }
}
