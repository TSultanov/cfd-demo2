...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    for (var face_offset = start; face_offset < end; face_offset++) {
        ...
        if (!is_boundary) {
            ...
            diag_00 += (0.0 - a_prod_scaled + a_prod_scaled * select(1.0, 0.0, bc_kind[boundary_type * 4u + 0u] == 1u)) * area;
            diag_01 += (normal.x * a_l + normal.x * a_r * select(1.0, 0.0, bc_kind[boundary_type * 4u + 1u] == 1u)) * area;
            diag_02 += (normal.y * a_l + normal.y * a_r * select(1.0, 0.0, bc_kind[boundary_type * 4u + 2u] == 1u)) * area;
            diag_03 += 0.0 * area;
            diag_10 += ((dp_drho_l * normal.x - u_l_x * u_n_l) * a_l + -u_l_x * inv_rho_l * mu_over_dist + ((dp_drho_r * normal.x - u_r_x * u_n_r) * a_r + -u_r_x * inv_rho_r * -mu_over_dist) * select(1.0, 0.0, bc_kind[boundary_type * 4u + 0u] == 1u)) * area;
            diag_11 += ((u_n_l + normal.x * (u_l_x + dp_dru_l)) * a_l + inv_rho_l * mu_over_dist - a_prod_scaled + ((u_n_r + normal.x * (u_r_x + dp_dru_r)) * a_r + inv_rho_r * -mu_over_dist + a_prod_scaled) * select(1.0, 0.0, bc_kind[boundary_type * 4u + 1u] == 1u)) * area;
            diag_12 += ((u_l_x * normal.y + dp_drv_l * normal.x) * a_l + (u_r_x * normal.y + dp_drv_r * normal.x) * a_r * select(1.0, 0.0, bc_kind[boundary_type * 4u + 2u] == 1u)) * area;
            diag_13 += (dp_dre_l * normal.x * a_l + dp_dre_r * normal.x * a_r * select(1.0, 0.0, bc_kind[boundary_type * 4u + 3u] == 1u)) * area;
            diag_20 += ((dp_drho_l * normal.y - u_l_y * u_n_l) * a_l + -u_l_y * inv_rho_l * mu_over_dist + ((dp_drho_r * normal.y - u_r_y * u_n_r) * a_r + -u_r_y * inv_rho_r * -mu_over_dist) * select(1.0, 0.0, bc_kind[boundary_type * 4u + 0u] == 1u)) * area;
            diag_21 += ((u_l_y * normal.x + dp_dru_l * normal.y) * a_l + (u_r_y * normal.x + dp_dru_r * normal.y) * a_r * select(1.0, 0.0, bc_kind[boundary_type * 4u + 1u] == 1u)) * area;
            diag_22 += ((u_n_l + normal.y * (u_l_y + dp_drv_l)) * a_l + inv_rho_l * mu_over_dist - a_prod_scaled + ((u_n_r + normal.y * (u_r_y + dp_drv_r)) * a_r + inv_rho_r * -mu_over_dist + a_prod_scaled) * select(1.0, 0.0, bc_kind[boundary_type * 4u + 2u] == 1u)) * area;
            diag_23 += (dp_dre_l * normal.y * a_l + dp_dre_r * normal.y * a_r * select(1.0, 0.0, bc_kind[boundary_type * 4u + 3u] == 1u)) * area;
