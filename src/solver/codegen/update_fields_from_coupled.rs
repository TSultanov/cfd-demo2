...
pub fn generate_update_fields_from_coupled_wgsl(layout: &StateLayout) -> String {
...
fn main_body(layout: &StateLayout) -> Block {
    ...
    let u_old_expr = state_vec2_expr(layout, "state", "idx", FIELD_U);
    let p_old_expr = state_scalar_expr(layout, "state", "idx", FIELD_P);

    let u_x_target = state_component_expr(layout, "state", "idx", FIELD_U, 0);
    let u_y_target = state_component_expr(layout, "state", "idx", FIELD_U, 1);
    let p_target = state_component_expr(layout, "state", "idx", FIELD_P, 0);
...
mod tests {
    ...
    fn update_fields_codegen_emits_state_arrays() {
        ...
        let wgsl = generate_update_fields_from_coupled_wgsl(&layout);
        assert!(wgsl.contains("state: array<f32>"));
        assert!(wgsl.contains("atomicMax"));
        assert!(wgsl.contains("arrayLength(&state) / 3u"));
    }
}
