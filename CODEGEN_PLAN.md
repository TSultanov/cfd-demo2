...
- Encapsulated generic-coupled kernels (assembly/update) into `GenericCoupledKernelsModule` and migrated `GpuGenericCoupledSolver::step()` to use `ModuleGraph` dispatch (`src/solver/gpu/modules/generic_coupled_kernels.rs`, `src/solver/gpu/plans/generic_coupled.rs`).
