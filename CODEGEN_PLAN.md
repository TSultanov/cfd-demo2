...
- Incompressible stepping is still orchestrated by legacy `GpuSolver::step_coupled_impl()` (called from the program via a single host node).
...
1. **Migrate incompressible scheduling into the program spec**
   - Replace the single “call legacy `step()`” host node with a program schedule (graphs + host nodes + control flow) mirroring the coupled solver loop.
2. **Normalize plan instance interfaces**
...
3. **Delete migrated legacy plan types**
...
6. **Drive scheme expansion end-to-end**
