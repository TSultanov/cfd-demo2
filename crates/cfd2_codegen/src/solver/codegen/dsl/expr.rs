...
use cfd2_ir::solver::dimensions::{UnitDimension, MulDim, DivDim, SqrtDim};
...
impl<A: UnitDimension, B: UnitDimension> std::ops::Mul<TypedExpr<B>> for TypedExpr<A> {
    ...
    fn mul(self, rhs: TypedExpr<B>) -> Self::Output {
        match (self.ty.shape, rhs.ty.shape) {
            ...
            _ => panic!("unsupported shape combination for *: {:?} and {:?}", self.ty.shape, rhs.ty.shape),
        }
    }
}
...
mod tests {
    ...
    use cfd2_ir::solver::dimensions::{Length, Time, Velocity, Dimensionless};
    ...
    fn typed_expr_multiplication_combines_dimensions() {
        let mass: TypedExpr<cfd2_ir::solver::dimensions::Mass> = TypedExpr::ident("m", DslType::f32());
    ...
    fn typed_expr_dimensionless_operations() {
        ...
        assert_eq!(<Dimensionless as UnitDimension>::UNIT, UnitDim::dimensionless());
    }
}
